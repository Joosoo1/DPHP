...
namespace onboardDetector {
    ...
    void UVtracker::read_bb(vector<cv::Rect> now_bb, vector<cv::Rect> now_bb_D,
                            vector<box3D>& box_3D) {
        // measurement history
    ...
    void UVtracker::check_status(vector<box3D>& box_3D) {
        for (size_t now_id = 0; now_id < this->now_bb.size(); now_id++) {
            ...
            for (size_t pre_id = 0; pre_id < this->pre_bb.size(); pre_id++) {
                ...
                float dist = std::sqrt(
                    std::pow((this->now_bb[now_id].x + 0.5 * this->now_bb[now_id].width)
                                 - (this->pre_bb[pre_id].x + 0.5 * this->pre_bb[pre_id].width),
                             2)
                    + std::pow((this->now_bb[now_id].y + 0.5 * this->now_bb[now_id].height
                                - (this->pre_bb[pre_id].y + 0.5 * this->pre_bb[pre_id].height)),
                               2));
                float metric =
                    std::sqrt(
                        std::pow(this->now_bb[now_id].width + this->pre_bb[pre_id].width, 2)
                        + std::pow(this->now_bb[now_id].height + this->pre_bb[pre_id].height, 2))
                    / 2;

                if (max(overlap.area() / float(this->now_bb[now_id].area()),
                        overlap.area() / float(this->pre_bb[pre_id].area()))
                        >= this->overlap_threshold
                    || dist <= metric) {
                    tracked = true;
                    ...
                    this->now_history[now_id].push_back(
                        cv::Point2f(this->now_bb[now_id].x + 0.5 * this->now_bb[now_id].width,
                                    this->now_bb[now_id].y + 0.5 * this->now_bb[now_id].height));
                    ...
                    if (this->now_bb_D[now_id].tl().x > 5 && this->now_bb_D[now_id].tl().y > 5
                        && this->now_bb_D[now_id].br().x < 635
                        && this->now_bb_D[now_id].br().y < 475) {
            ...
            if (!tracked) {
                ...
                // add current 3D box to box history only if when it is full in the FOV. otherwise, clear the history and track again
                if (this->now_bb_D[now_id].tl().x > 5 && this->now_bb_D[now_id].tl().y > 5
                    && this->now_bb_D[now_id].br().x < 635 && this->now_bb_D[now_id].br().y < 475) {
                    this->now_box_3D_history[now_id].push_back(this->now_box_3D[now_id]);
                }
            }
        }
    ...
    void UVdetector::extract_U_map() {
        ...
        cv::Mat depth_low_res_temp =
            cv::Mat::zeros(depth_rescale.rows, depth_rescale.cols, CV_8UC1);
        ...
        for (int col = 0; col < depth_rescale.cols; col++) {
            for (int row = 0; row < depth_rescale.rows; row++) {
                ...
                depth_rescale_val =
                    int((float(depth_rescale.at<unsigned short>(row, col)) / this->depthScale_)
                        * 1000.0);
    ...
    void UVdetector::extract_bb() {
        ...
        for (int row = 0; row < this->U_map.rows; row++) {
            for (int col = 0; col < this->U_map.cols; col++) {
                ...
                if (this->U_map.at<uchar>(row, col)
                    >= u_min)  // num of points at this depth >= u_min
                {
                    // update current line info
                    length_line++;
                    sum_line += this->U_map.at<uchar>(row, col);
                    max_line = (this->U_map.at<uchar>(row, col) > max_line)
                        ? this->U_map.at<uchar>(row, col)
                        : max_line;
                }
                ...
                if (this->U_map.at<uchar>(row, col) < u_min || col == this->U_map.cols - 1) {
                    ...
                    if (length_line > this->min_length_line
                        && sum_line > this->threshold_line * max_line) {
                        seg_id++;
                        ...
                        if (row != 0) {
                            ...
                            for (int c = col - length_line; c < col - 1; c++) {
                                if (mask[row - 1][c] != 0) {
                                    ...
                                    // mask stores seg_ids
                                    if (UVboxes[mask[row - 1][c] - 1].toppest_parent_id
                                        < UVboxes.back().toppest_parent_id) {
                                        UVboxes.back().toppest_parent_id =
                                            UVboxes[mask[row - 1][c] - 1].toppest_parent_id;
                                    } else {
                                        int temp = UVboxes[mask[row - 1][c] - 1].toppest_parent_id;
                                        for (size_t b = 0; b < UVboxes.size(); b++) {
                                            UVboxes[b].toppest_parent_id =
                                                (UVboxes[b].toppest_parent_id == temp)
                                                ? UVboxes.back().toppest_parent_id
                                                : UVboxes[b].toppest_parent_id;
                                        }
                                    }
    ...
    void UVdetector::extract_3Dbox() {
        ...
        for (size_t b = 0; b < this->bounding_box_U.size(); b++) {
            ...
            depth_in_far = depth_of_depth * 1.3
                + depth_in_near;  // assumed depth was truncated because of occlusion
            ...
            for (int i = x; i < x + width; i++) {                  // for several middle coloums
                for (int j = 0; j < depth_resize.rows - 1; j++) {  // for each row
                    depth_resize_val =
                        (float(depth_resize.at<unsigned short>(j, i)) / this->depthScale_) * 1000.0;
                    if (float(depth_resize_val) >= depth_in_near
                        && depth_resize_val <= depth_in_far) {
                        for (int check = 0; check < num_check;
                             check++) {  // check some more points in the coloum
                            depth_resize_val =
                                (float(depth_resize.at<unsigned short>(j + check + 1, i))
                                 / this->depthScale_)
                                * 1000.0;
    ...
    void UVdetector::display_U_map() {
        ...
        if (this->show_bounding_box_U) {
            ...
            for (size_t b = 0; b < this->bounding_box_U.size(); b++) {
                cv::Rect final_bb = cv::Rect(
                    this->bounding_box_U[b].tl(),
                    cv::Size(this->bounding_box_U[b].width, 2 * this->bounding_box_U[b].height));
    ...
    void UVdetector::extract_bird_view() {
        ...
        for (size_t b = 0; b < this->bounding_box_U.size(); b++) {
            ...
            float bb_x =
                bb_depth * (this->bounding_box_U[b].tl().x / this->col_scale - this->px) / this->fx;
            float bb_y = bb_depth
                - 0.5
                    * bb_height;  // assume farthest depth value is the depth of center point, then assume detected depth difference is the depth of the whole body. y is the depth direction
    ...
    void UVdetector::display_bird_view() {
        ...
        cv::Point2f left_end_to_center =
            cv::Point2f(this->bird_view.rows * (0 - this->px) / this->fx, -this->bird_view.rows);
        cv::Point2f right_end_to_center = cv::Point2f(
            this->bird_view.rows * (this->depth.cols - this->px) / this->fx, -this->bird_view.rows);
        ...
        for (size_t b = 0; b < this->bounding_box_U.size(); b++) {
            ...
            // draw center
            cv::Point2f bb_center =
                cv::Point2f(final_bb.x + 0.5 * final_bb.width, final_bb.y + 0.5 * final_bb.height);
    ...
    void UVdetector::add_tracking_result() {
        ...
        for (size_t b = 0; b < this->tracker.now_bb.size(); b++) {
            // change coordinates
            cv::Point2f estimated_center = cv::Point2f(this->tracker.now_filter[b].output(0),
                                                       this->tracker.now_filter[b].output(1));
            ...
            // draw bounding box
            cv::Point2f bb_size = cv::Point2f(this->tracker.now_filter[b].output(4),
                                              this->tracker.now_filter[b].output(5));
            rectangle(this->bird_view,
                      cv::Rect(estimated_center - 0.5 * bb_size, estimated_center + 0.5 * bb_size),
                      cv::Scalar(0, 255, 0), 3, 8, 0);
            // draw velocity
            cv::Point2f velocity = cv::Point2f(this->tracker.now_filter[b].output(2),
                                               this->tracker.now_filter[b].output(3));
            velocity.y =
                -velocity.y;  // y direction in birdvie map is in opposite of opencv::line default
            ...
            line(this->bird_view, estimated_center, estimated_center + velocity,
