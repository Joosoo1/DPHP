...
namespace onboardDetector {
    ...
    inline geometry_msgs::Quaternion quaternion_from_rpy(double roll, double pitch, double yaw) {
        if (yaw > PI_const) { yaw = yaw - 2 * PI_const; }
        tf2::Quaternion quaternion_tf2;
        quaternion_tf2.setRPY(roll, pitch, yaw);
        const geometry_msgs::Quaternion quaternion = tf2::toMsg(quaternion_tf2);
        return quaternion;
    }
    ...
    inline void rpy_from_quaternion(const geometry_msgs::Quaternion& quat, double& roll,
