            ...
            for (size_t i=0 ; i<this->obstacleHist_.size() ; ++i){
				if (this->isObstacleInSensorRange(this->obstacleHist_[i][0],2*M_PI)){
					...
					for (size_t j=0; j<this->obstacleHist_[i].size() ; ++j){
						...
						if (j > 0) {
							int k = std::min(j, (size_t)3); // Use 3 frames for acceleration calculation
							if (j >= k) {
								double accX = (vel(0) - this->obstacleHist_[i][j-k].Vx) / (0.1 * k); // Assuming 0.1s time step
								double accY = (vel(1) - this->obstacleHist_[i][j-k].Vy) / (0.1 * k);
