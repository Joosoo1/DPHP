...
class TrajpredDataset(Dataset):
                ...
                for i in range(0, len(pedestrian_track) - chunk_length, self.stride):
                    ...
                    for t in range(self.tbptt + 1):
                        ...
                        if processor.target_mode == 'position':
                            cv_pred = np.array(ego_history_chunk[['X', 'Y']].values[-1] + np.array(
                                [np.arange(1, pred_horizon + 1), np.arange(1, pred_horizon + 1)]).T *
                                               ego_history_chunk[['Vx', 'Vy']].values[-1] * (1 / self.frequency))
                            example['cv_ade'] = np.mean(
                                np.linalg.norm(ego_future_chunk[['X', 'Y']].values - cv_pred, axis=1))
                            example['cv_fde'] = np.linalg.norm(ego_future_chunk[['X', 'Y']].values[-1] - cv_pred[-1])
                        else:
                            example['cv_ade'] = np.mean(np.linalg.norm(
                                ego_future_chunk[['Vx', 'Vy']].values - ego_history_chunk[['Vx', 'Vy']].values[-1],
                                axis=1))
                            example['cv_fde'] = np.linalg.norm(
                                ego_future_chunk[['Vx', 'Vy']].values[-1] - ego_history_chunk[['Vx', 'Vy']].values[-1])
    ...
    def _load_experiment_df(self, f):
        ...
        tracks_frequency = int(
            1000 / np.min(tracks_frequencies[tracks_frequencies != 0]))  # int(np.mean(np.diff(tracks.Frame.values)))
