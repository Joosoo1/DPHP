...
namespace grid_world_ns {
    ...
    GridWorld::GridWorld(const int row_num, const int col_num, const int level_num,
                         const double cell_size, const double cell_height,
                         const int nearby_grid_num)
        : kRowNum(row_num),
          kColNum(col_num),
          kLevelNum(level_num),
          kCellSize(cell_size),
    ...
    void GridWorld::ReadParameters(ros::NodeHandle& nh) {
        ...
        kLevelNum = misc_utils_ns::getParam<int>(
            nh, "kGridWorldZNum",
            121);  // 划分的每个维度元素个数（121^3=1771561），即1771561个子空间

        const int viewpoint_number =
            misc_utils_ns::getParam<int>(nh, "viewpoint_manager/number_x", 40);
        // 视点采样数量 number_x * number_y * number_z(40*40*1)
        const auto viewpoint_resolution =
        ...
        kCellHeight =
            misc_utils_ns::getParam<double>(nh, "kGridWorldCellHeight", 8.0);  // 每个子空间的高度

        KNearbyGridNum = misc_utils_ns::getParam<int>(nh, "kGridWorldNearbyGridNum",
                                                      5);  // x、y方向上的邻居子空间数量，z轴默认为1
        ...
        kCellExploringToCoveredThr =
            misc_utils_ns::getParam<int>(nh, "kCellExploringToCoveredThr", 1);
        kCellCoveredToExploringThr =
            misc_utils_ns::getParam<int>(nh, "kCellCoveredToExploringThr", 10);
        kCellExploringToAlmostCoveredThr =
            misc_utils_ns::getParam<int>(nh, "kCellExploringToAlmostCoveredThr", 10);
        kCellAlmostCoveredToExploringThr =
            misc_utils_ns::getParam<int>(nh, "kCellAlmostCoveredToExploringThr", 20);
        kCellUnknownToExploringThr =
            misc_utils_ns::getParam<int>(nh, "kCellUnknownToExploringThr", 1);
        kCellSemiExploredToExploringThr =
            misc_utils_ns::getParam<int>(nh, "kCellSemiExploredToExploringThr", 0.99);
    }
    ...
    void GridWorld::UpdateNeighborCells(const geometry_msgs::Point& robot_position) {
            ...
            for (int i = 0; i < kRowNum; i++) {
                for (int j = 0; j < kColNum; j++) {
                    for (int k = 0; k < kLevelNum; k++) {
                        ...
                        subspaces_->GetCell(i, j, k).SetRoadmapConnectionPoint(
                            subspace_center_position);
                    }
                }
            }
        }
        ...
        const int N = KNearbyGridNum / 2;  // 5/2=2.5，取整为2，xy平面临近的cell范围
        constexpr int M = 1;
        ...
        for (const auto& cell_ind : neighbor_cell_indices_) {
            ...
            if (std::find(prev_neighbor_cell_indices.begin(), prev_neighbor_cell_indices.end(),
                          cell_ind)
    ...
    void GridWorld::UpdateRobotPosition(const geometry_msgs::Point& robot_position) {
        ...
        const int robot_cell_ind =
            GetCellInd(robot_position_.x, robot_position_.y, robot_position_.z);
    ...
    void GridWorld::UpdateCellKeyposeGraphNodes(
        const std::unique_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph) const {
        const std::vector<int> keypose_graph_connected_node_indices =
            keypose_graph->GetConnectedGraphNodeIndices();
        // 获取可连接的keypose节点索引
        ...
        // 遍历keypose_graph的每个节点，如果该节点对应的cell的状态为EXPLORING/SEMI_EXPLORED，则将该节点添加到该cell的keypose节点索引中
        for (const auto& node_ind : keypose_graph_connected_node_indices) {
            const geometry_msgs::Point node_position =
                keypose_graph->GetNodePosition(node_ind);  // 获取节点位置
            const int cell_ind = GetCellInd(node_position.x, node_position.y,
                                            node_position.z);  // 获取该节点位置所对应的cell索引
    ...
    bool GridWorld::AreNeighbors(const int cell_ind1, const int cell_ind2) const {
        const Eigen::Vector3i cell_sub1 = subspaces_->Ind2Sub(cell_ind1);
        const Eigen::Vector3i cell_sub2 = subspaces_->Ind2Sub(cell_ind2);
        Eigen::Vector3i diff = cell_sub1 - cell_sub2;
        if (std::abs(diff.x()) + std::abs(diff.y()) + std::abs(diff.z()) == 1) { return true; }
        return false;
    }
    ...
    void GridWorld::GetCellSub(int& row_idx, int& col_idx, int& level_idx, const double qx,
                               const double qy, const double qz) const {
    ...
    void GridWorld::GetMarker(visualization_msgs::Marker& marker) const {
        ...
        for (int i = 0; i < kRowNum; i++) {
            for (int j = 0; j < kColNum; j++) {
                for (int k = 0; k < kLevelNum; k++) {
                    const int cell_ind = subspaces_->Sub2Ind(i, j, k);
    ...
    void GridWorld::GetVisualizationCloud(
    ...
    void GridWorld::GetNeighborCellIndices(const geometry_msgs::Point& position,
                                           const Eigen::Vector3i& neighbor_range) {
        const Eigen::Vector3i center_cell_sub =
            GetCellSub(Eigen::Vector3d(position.x, position.y, position.z));
    ...
    void GridWorld::SetCellRobotPosition(const int cell_ind,
                                         const geometry_msgs::Point& robot_position) const {
        MY_ASSERT(subspaces_->InRange(cell_ind));
    ...
    // 获取
    void GridWorld::getSemiExploredCellIndices(
        ...
        for (const auto& position : semi_dynamic_frontier_positions) {
            ...
            if (subspaces_->InRange(index)) {
                subspaces_->GetCell(index).SetStatus(CellStatus::SEMI_EXPLORED);
                semi_explored_cell_indices_.push_back(index);
            }
        }
    }

    void GridWorld::UpdateCellStatus(
        ...
        for (int i = 0; i < subspaces_->GetCellNumber(); ++i) {
            ...
            {
                covered_count++;
            } else if (subspaces_->GetCell(i).GetStatus()
                       == CellStatus::SEMI_EXPLORED)  // semi-explored
            {
                semi_explored_count++;
            }
        }
        ...
        for (const auto& viewpoint_ind : viewpoint_manager->candidate_indices_) {
            ...
            Eigen::Vector3i sub = subspaces_->Pos2Sub(
            ...
            {
                const int cell_ind = subspaces_->Sub2Ind(sub);  // 获取该子空间的全局索引
                AddViewPointToCell(cell_ind, viewpoint_ind);  // 将该Viewpoint添加到该cell中
                viewpoint_manager->SetViewPointCellInd(viewpoint_ind,
                                                       cell_ind);  // 设置该Viewpoint对应的cell索引
            } else {
        ...
        for (const auto& cell_ind : neighbor_cell_indices_) {
                     ...
                     .GetViewPointIndices())  // 遍历邻近cell中的所有Viewpoint
            {
                ...
                int frontier_score =
                    viewpoint_manager->GetViewPointCoveredFrontierPointNum(viewpoint_ind);
            ...
            // Unseen to Exploring
                ...
                almost_covered_cell_indices_.erase(
                    std::remove(almost_covered_cell_indices_.begin(),
                                almost_covered_cell_indices_.end(), cell_ind),
            ...
            else if (subspaces_->GetCell(cell_ind).GetStatus()
                     == CellStatus::SEMI_EXPLORED)  // todo
            {
                subspaces_->GetCell(cell_ind).SetStatus(CellStatus::EXPLORING);
            }
    ...
    double GridWorld::GetSemiCellTransitionProbability(const int& cell_ind) {
        constexpr double base_prob = 0.05;
        const double detection_frequency_factor = CalculateDoorDetectionFrequency(cell_ind);
        const double time_factor = CalculateTimeBasedFactor();
        const double distance_factor = CalculateDistanceFactor(cell_ind);

        const double transition_prob = base_prob + detection_frequency_factor * 0.4
            + time_factor * 0.2 + distance_factor * 0.1;
    ...
    double GridWorld::CalculateDoorDetectionFrequency(int cell_ind) {
        if (std::find(semi_explored_cell_indices_.begin(), semi_explored_cell_indices_.end(),
                      cell_ind)
            != semi_explored_cell_indices_.end()) {
            return 0.7;
        }
    ...
    double GridWorld::CalculateTimeBasedFactor() {
        // As a simple approximation, return a moderate value
        return 0.3;
    }

    // 计算距离因素
    double GridWorld::CalculateDistanceFactor(const int cell_ind) const {
        // Get the position of the cell
        const geometry_msgs::Point cell_position = GetCellPosition(cell_ind);

        // Calculate Euclidean distance to robot
        const double dx = cell_position.x - robot_position_.x;
        const double dy = cell_position.y - robot_position_.y;
        const double dz = cell_position.z - robot_position_.z;
        const double distance = sqrt(dx * dx + dy * dy + dz * dz);

        const double normalized_distance = distance / kCellSize;

        const double distance_factor = exp(-normalized_distance * 0.5);
    ...
    exploration_path_ns::ExplorationPath GridWorld::SolveGlobalTSP(
        ...
        keypose_graph->GetClosestConnectedNodeIndAndDistance(robot_position_, closest_node_ind,
                                                             closest_node_dist);
                ...
                if (subspaces_->GetCell(cell_ind).IsRoadmapConnectionPointSet()) {
                    ...
                    if (viewpoint_manager->InLocalPlanningHorizon(roadmap_connection_point)) {
                        double dist_to_robot =
                            (roadmap_connection_point - eigen_robot_position).norm();
            ...
            if (subspaces_->GetCell(i).GetStatus() == CellStatus::EXPLORING) {
                ...
                if (std::find(neighbor_cell_indices_.begin(), neighbor_cell_indices_.end(), i)
                    ...
                    } else  // 如果keypose_graph不为空
                    {
                        ...
                        connection_point_geo.z =
                            connection_point.z();  // 将连接点转成geometry_msgs::Point类型
                        ...
                        } else  // 如果不可达，则遍历该cell的keypose_graph节点，找到最接近连接点的节点
                        {
                            ...
                            for (const auto& node_ind :
                                 subspaces_->GetCell(i).GetGraphNodeIndices()) {
                                geometry_msgs::Point node_position =
                                    keypose_graph->GetNodePosition(node_ind);
                                double dist = misc_utils_ns::PointXYZDist<geometry_msgs::Point,
                                                                          geometry_msgs::Point>(
                            ...
                            if (min_dist_node_ind >= 0
                                && min_dist_node_ind < keypose_graph->GetNodeNum()) {
                                reachable = true;
                                connection_point_geo =
                                    keypose_graph->GetNodePosition(min_dist_node_ind);
                            }
                        }
        ...
        if (exploring_cell_indices.empty()) {
            ...
            } else  // 如果keypose_graph不为空
            {
                ...
                keypose_graph->GetShortestPath(global_path_robot_position, home_position, true,
                                               return_home_path, false);
                //
        ...
        for (int i = 0; i < exploring_cell_positions.size();
             i++)  // 遍历exploring_cell_positions的所有位置点
        {
            for (int j = 0; j < i; j++) {
                ...
                if (!use_keypose_graph_ || keypose_graph == nullptr
                    ...
                    distance_matrix[i][j] = static_cast<int>(
                        10
                ...
                } else  // 若存在keypose_graph，则求取在keypose_graph中两点之间的最短距离
                {
                    ...
                    distance_matrix[i][j] =
                        static_cast<int>(10
                                         * keypose_graph->GetShortestPath(
                                             exploring_cell_positions[i],
        ...
        if (!use_keypose_graph_ || keypose_graph == nullptr || keypose_graph->GetNodeNum() == 0) {
            ...
            for (int cell_ind : node_index) {
                ...
                ordered_cell_indices.push_back(
                    exploring_cell_indices[cell_ind]);  // 每个cell的访问顺序
            }
        } else {
            ...
            for (int i = 0; i < node_index.size() - 1; i++) {
                ...
                exploration_path_ns::Node node(
                    Eigen::Vector3d(cur_position.x, cur_position.y, cur_position.z));
                ...
                if (keypose_path.poses.size() >= 2) {
                    for (int j = 1; j < keypose_path.poses.size() - 1; j++) {
                        ...
                        exploration_path_ns::Node keypose_node(
                            node_position, exploration_path_ns::NodeType::GLOBAL_VIA_POINT);
                        keypose_node.keypose_graph_node_ind_ =
                            static_cast<int>(keypose_path.poses[i].pose.orientation.x);
                        global_path.Append(
                            keypose_node);  // 将两个GLOBAL_VIEWPOINT之间的路径添加到全局路径中
                    }
                }
            }
    ...
    void GridWorld::AddPathsInBetweenCells(
        ...
        for (int cell_ind : neighbor_cell_indices_) {
            ...
            std::vector<int> candidate_viewpoint_indices =
                subspaces_->GetCell(cell_ind).GetViewPointIndices();
            // 如果该cell内的视点不为空
            if (!candidate_viewpoint_indices.empty()) {
                ...
                geometry_msgs::Point min_dist_viewpoint_position =
                    viewpoint_manager->GetViewPointPosition(
                        min_dist_viewpoint_ind);  // 获取距离该cell中心最近的视点的位置

                subspaces_->GetCell(cell_ind).SetRoadmapConnectionPoint(
                    Eigen::Vector3d(min_dist_viewpoint_position.x, min_dist_viewpoint_position.y,
                                    min_dist_viewpoint_position.z));
                // 设置该视点位置为该cell与roadmap的连接点

                subspaces_->GetCell(cell_ind).SetRoadmapConnectionPointSet(
                    true);  // 反馈已将该cell与roadmap的连接
            }
        }
        ...
        for (int from_cell_ind : neighbor_cell_indices_) {
            ...
            Eigen::Vector3d from_cell_roadmap_connection_position =
                subspaces_->GetCell(from_cell_ind)
                    .GetRoadmapConnectionPoint();  // 获取该cell与roadmap的连接点位置
                ...
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                            ...
                            == 1)  // 栅格空间内与from_cell_ind相邻的cell(6个),上下左右前后，距离为一个单元格cell
                        {
                            ...
                            {
                                int neighbor_ind =
                                    subspaces_->Sub2Ind(neighbor_sub);  // 获取该cell的一维全局索引
                                nearby_cell_indices.push_back(
                                    neighbor_ind);  // 添加到nearby_cell_indices中
                            }
                        }
                    }
                }
            ...
            for (int to_cell_ind : nearby_cell_indices) {
                ...
                {
                    ROS_ERROR_STREAM("Cell " << from_cell_ind << " and " << to_cell_ind
                                             << " are not neighbors");
                }
                ...
                Eigen::Vector3d to_cell_roadmap_connection_position =
                    subspaces_->GetCell(to_cell_ind)
                        .GetRoadmapConnectionPoint();  // 获取该邻近cell与roadmap的连接点位置
                ...
                {
                    path_in_between =
                        misc_utils_ns::SimplifyPath(path_in_between);  // 将相连路径简化
    ...
    // 判断路径是否有效？
    bool GridWorld::PathValid(const nav_msgs::Path& path, const int from_cell_ind,
                              const int to_cell_ind) const {
    ...
    bool GridWorld::HasDirectKeyposeGraphConnection(
        const std::unique_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph,
        ...
        const double max_path_length = kCellSize * 2;
        nav_msgs::Path path;
        // 在keypose_graph中能否搜索到从start_position到goal_position之间的路径，路径长度不超过max_path_length即两倍cell边长
        const bool found_path = keypose_graph->GetShortestPathWithMaxLength(
    ...
    void GridWorld::UpdateSemiDynamicFrontiers(const geometry_msgs::Point& robot_position) {
        ...
        for (const int cell_ind : semi_explored_cell_indices_) {
            Cell& cell = subspaces_->GetCell(cell_ind);
            geometry_msgs::Point cell_center = cell.GetPosition();

            // 计算cell到机器人的距离
            const double distance = sqrt(pow(cell_center.x - robot_position.x, 2)
                                         + pow(cell_center.y - robot_position.y, 2)
                                         + pow(cell_center.z - robot_position.z, 2));

            // 只考虑在一定距离范围内的cell
            constexpr double kSemiDynamicFrontierRange = 15.0;  // 半动态前沿点考虑范围?
    ...
    void GridWorld::UpdateSemiExploredCellStatus() {
        // Iterate through all semi-explored cells to check if they should transition to EXPLORING
        auto it = semi_explored_cell_indices_.begin();
        while (it != semi_explored_cell_indices_.end()) {
            int cell_ind = *it;

            // Check if the cell is still within bounds
            if (!IndInBound(cell_ind)) {
                it = semi_explored_cell_indices_.erase(it);
                continue;
            }
            // Calculate transition probability
            const double transition_prob = GetSemiCellTransitionProbability(cell_ind);

            // If probability exceeds threshold, transition to EXPLORING state
            if (transition_prob > kCellSemiExploredToExploringThr) {
                subspaces_->GetCell(cell_ind).SetStatus(CellStatus::EXPLORING);
                // Remove from semi-explored list
                it = semi_explored_cell_indices_.erase(it);
            } else {
                ++it;
            }
    ...
    void GridWorld::PrintSemiExploredCellsInfo() {
        ...
        for (int cell_ind : semi_explored_cell_indices_) {
            const double transition_prob = GetSemiCellTransitionProbability(cell_ind);
            const geometry_msgs::Point pos = GetCellPosition(cell_ind);
