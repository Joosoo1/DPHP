...
namespace misc_utils_ns
{
    ...
    void KeyposeToMap(CloudType& cloud, const nav_msgs::Odometry::ConstPtr& keypose)
    {
        ...
        const auto sin_roll = static_cast<float>(sin(roll));
        const auto cos_roll = static_cast<float>(cos(roll));
        const auto sin_pitch = static_cast<float>(sin(pitch));
        const auto cos_pitch = static_cast<float>(cos(pitch));
        ...
        for (auto& point : cloud->points)
        {
            ...
            const float x2 = x1;
            const float y2 = y1 * cos_roll - z1 * sin_roll;
            const float z2 = y1 * sin_roll + z1 * cos_roll;

            const float x3 = x2 * cos_pitch + z2 * sin_pitch;
            const float y3 = y2;
            const float z3 = -x2 * sin_pitch + z2 * cos_pitch;

            const float x4 = x3 * cos_yaw - y3 * sin_yaw;
            const float y4 = x3 * sin_yaw + y3 * cos_yaw;
            const float z4 = z3;
    ...
    /// Function to check if three points are collinear in the xy-plane，检测三点是否共线
    ...
    /// Function to calculate the overlap of two angle intervals [s1, e1] and [s2, e2]. s1(2)->e1(2) counter-clockwise
    /// and
    //  all angles shoud be in [-pi, pi] \param s1 starting angle of the first interval \param e1 end angle of the
                             ...
                             const geometry_msgs::Point& line_segment_end)
    {
        ...
        // vector end to start
        const double v_x = line_segment_end.x - line_segment_start.x;
        const double v_y = line_segment_end.y - line_segment_start.y;

        // vector start to point
        const double w_x = point.x - line_segment_start.x;
                             ...
                             const geometry_msgs::Point& line_segment_end)
    {
        ...
        const double v_x = line_segment_end.x - line_segment_start.x;
        const double v_y = line_segment_end.y - line_segment_start.y;
        const double v_z = line_segment_end.z - line_segment_start.z;
