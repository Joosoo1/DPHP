...
#include "explorer/implementation/sensor_coverage_planner_ground.h"
#include "explorer/utility/misc_utils.h"
#include "explorer/utility/graph.h"
// 从配置文件中读取参数
namespace sensor_coverage_planner_3d_ns
{
    ...
    void PlannerData::Initialize(ros::NodeHandle& nh, ros::NodeHandle& nh_p)
    {
        ...
        pd_.GetRegisteredScanStack() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZ>>(nh, "registered_scan_stack", kWorldFrameID);
        pd_.GetRegisteredCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "registered_cloud", kWorldFrameID);
        pd_.GetLargeTerrainCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "terrain_cloud_large", kWorldFrameID);
        pd_.GetTerrainCollisionCloud() = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
            nh, "terrain_collision_cloud", kWorldFrameID);
        pd_.GetTerrainExtCollisionCloud() = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
            nh, "terrain_ext_collision_cloud", kWorldFrameID);
        pd_.GetViewpointVisCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "viewpoint_vis_cloud", kWorldFrameID);
        pd_.GetGridWorldVisCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "grid_world_vis_cloud", kWorldFrameID);
        pd_.GetExplorationPathCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "bspline_path_cloud", kWorldFrameID);

        pd_.GetSelectedViewpointVisCloud() = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
            nh, "selected_viewpoint_vis_cloud", kWorldFrameID);
        pd_.GetExploringCellVisCloud() = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
            nh, "exploring_cell_vis_cloud", kWorldFrameID);
        pd_.GetCollisionCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "collision_cloud", kWorldFrameID);
        pd_.GetLookaheadPointCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "lookahead_point_cloud", kWorldFrameID);
        pd_.GetKeyposeGraphVisCloud() =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(nh, "keypose_graph_cloud", kWorldFrameID);
        pd_.GetViewpointInCollisionCloud() = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
            nh, "viewpoint_in_collision_cloud_", kWorldFrameID);
        pd_.GetPointCloudManagerNeighborCloud() = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
            nh, "pointcloud_manager_cloud", kWorldFrameID);
        pd_.GetReorderedGlobalSubspaceCloud() = std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
            nh, "reordered_global_subspace_cloud", kWorldFrameID);

        /*local planner*/
        pd_.GetPlanningEnv() = std::make_unique<planning_env_ns::PlanningEnv>(nh, nh_p);
        pd_.GetViewpointManager() = std::make_shared<viewpoint_manager_ns::ViewPointManager>(nh_p);
        pd_.GetLocalCoveragePlanner() = std::make_unique<local_coverage_planner_ns::LocalCoveragePlanner>(nh_p);
        pd_.GetLocalCoveragePlanner()->SetViewPointManager(
            std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())); // local_coverage_planner与viewpoint_manager关联，用于局部规划

        /*global planner*/
        pd_.GetKeyposeGraph() = std::make_unique<keypose_graph_ns::KeyposeGraph>(nh_p);
        pd_.GetGridWorld() = std::make_unique<grid_world_ns::GridWorld>(nh_p);
        std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->SetUseKeyposeGraph(true); // GridWorld同keypose_graph关联，用于全局规划

        /*可视化*/
        pd_.GetVisualizer() = std::make_unique<explorer_visualizer_ns::TAREVisualizer>(nh, nh_p);

        // keypose_graph的节点可视化
        pd_.GetKeyposeGraphNodeMarker() =
            std::make_unique<misc_utils_ns::Marker>(nh, "keypose_graph_node_marker", kWorldFrameID);
        pd_.GetKeyposeGraphNodeMarker()->SetType(visualization_msgs::Marker::POINTS);
        pd_.GetKeyposeGraphNodeMarker()->SetScale(0.4, 0.4, 0.1);
        pd_.GetKeyposeGraphNodeMarker()->SetColorRGBA(1.0, 0.0, 0.0, 1.0);

        // keypose_graph的边可视化
        pd_.GetKeyposeGraphEdgeMarker() =
            std::make_unique<misc_utils_ns::Marker>(nh, "keypose_graph_edge_marker", kWorldFrameID);
        pd_.GetKeyposeGraphEdgeMarker()->SetType(visualization_msgs::Marker::LINE_LIST);
        pd_.GetKeyposeGraphEdgeMarker()->SetScale(0.05, 0.0, 0.0);
        pd_.GetKeyposeGraphEdgeMarker()->SetColorRGBA(1.0, 1.0, 0.0, 0.9);

        // 禁止边界可视化
        pd_.GetNogoBoundaryMarker() = std::make_unique<misc_utils_ns::Marker>(nh, "nogo_boundary_marker", kWorldFrameID);
        pd_.GetNogoBoundaryMarker()->SetType(visualization_msgs::Marker::LINE_LIST);
        pd_.GetNogoBoundaryMarker()->SetScale(0.05, 0.0, 0.0);
        pd_.GetNogoBoundaryMarker()->SetColorRGBA(1.0, 0.0, 0.0, 0.8);

        // grid_world立体框可视化
        pd_.GetGridWorldMarker() = std::make_unique<misc_utils_ns::Marker>(nh, "grid_world_marker", kWorldFrameID);
        pd_.GetGridWorldMarker()->SetType(visualization_msgs::Marker::CUBE_LIST);
        pd_.GetGridWorldMarker()->SetScale(1.0, 1.0, 1.0);
        pd_.GetGridWorldMarker()->SetColorRGBA(1.0, 0.0, 0.0, 0.8);

        /*初始化*/
        pd_.GetInitialPosition().x() = 0.0;
        pd_.GetInitialPosition().y() = 0.0;
        pd_.GetInitialPosition().z() = 0.0;
        pd_.GetCurKeyposeNodeInd() = 0;
        pd_.GetRobotYaw() = 0.0;
        pd_.GetLookaheadPointDirection() = Eigen::Vector3d(1.0, 0.0, 0.0);
        pd_.GetMovingDirection() = Eigen::Vector3d(1.0, 0.0, 0.0);
        pd_.GetMovingForward() = true; // 默认为前进

        Eigen::Vector3d viewpoint_resolution = std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->GetResolution();
        double add_non_keypose_node_min_dist = std::min(viewpoint_resolution.x(), viewpoint_resolution.y()) / 2;
        std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->SetAddNonKeyposeNodeMinDist() =
            add_non_keypose_node_min_dist; // 设置非关键节点最小距离，即普通点

        pd_.GetRobotPosition().x = 0;
        pd_.GetRobotPosition().y = 0;
        pd_.GetRobotPosition().z = 0;

        pd_.GetLastRobotPosition() = pd_.GetRobotPosition();
    }
    ...
    void SensorCoveragePlanner3D::RegisteredScanCallback(const sensor_msgs::PointCloud2ConstPtr& registered_scan_msg)
    {
        ...
        *(pd_.registered_scan_stack_->cloud_) += *(registered_scan_tmp); // 将点云加入点云堆栈中
        ...
        if (registered_cloud_count_ == 0) // 每过5帧标记一帧为关键帧
        {
            ...
            pd_.GetCurKeyposeNodeInd() = std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->AddKeyposeNode(pd_.GetKeypose(), *(std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())));
    ...
    void SensorCoveragePlanner3D::UpdateKeyposeGraph()
    {
        ...
        std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->GetMarker(pd_.GetKeyposeGraphNodeMarker()->marker_, pd_.GetKeyposeGraphEdgeMarker()->marker_);
        // pd_.keypose_graph_node_marker_->Publish();
        pd_.GetKeyposeGraphEdgeMarker()->Publish();
        pd_.GetKeyposeGraphVisCloud()->cloud_->clear();
        //
        std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->CheckLocalCollision(pd_.GetRobotPosition(), std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager()));
        std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->CheckConnectivity(pd_.GetRobotPosition());
        std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->GetVisualizationCloud(pd_.GetKeyposeGraphVisCloud()->cloud_);
        pd_.GetKeyposeGraphVisCloud()->Publish();
    ...
    int SensorCoveragePlanner3D::UpdateViewPoints()
    {
        ...
        if (pp_.kUseTerrainHeight)
        {
            std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->SetViewPointHeightWithTerrain(
                pd_.GetLargeTerrainCloud()->cloud_); // 根据扩展地形图点云获取视点的高度
        }

        if (pp_.kCheckTerrainCollision)
        {
            *(pd_.GetCollisionCloud()->cloud_) += *(pd_.GetTerrainCollisionCloud()->cloud_);
            *(pd_.GetCollisionCloud()->cloud_) += *(pd_.GetTerrainExtCollisionCloud()->cloud_);
        } // 汇总障碍物点云

        std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->CheckViewPointCollision(
            pd_.GetCollisionCloud()->cloud_); // 输入障碍物点云，检查视点是否与障碍物碰撞
        std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->CheckViewPointLineOfSight(); // 检查视点是否在视线范围内
        std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->CheckViewPointConnectivity(); // 检测视点与其它视点之间的连通性

        int viewpoint_candidate_count = std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->GetViewPointCandidate(); // 获取候选视点的数量

        UpdateVisitedPositions(); // 更新无人车已经访问过的位置
        std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->UpdateViewPointVisited(pd_.GetVisitedPositions()); // 更新视点是否被访问过
        std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->UpdateViewPointVisited(std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())); // 更新栅格地图中的cell是否被访问过
    ...
    void SensorCoveragePlanner3D::UpdateCoveredAreas(int& uncovered_point_num, int& uncovered_frontier_point_num)
    {
        ...
        std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())->UpdateCoveredArea(pd_.GetRobotViewpoint(), std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())); // 更新已覆盖区域
        ...
        std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())->GetUncoveredArea(std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager()), uncovered_point_num, uncovered_frontier_point_num);
        get_uncovered_area_timer.Stop(false);
        // 发布未覆盖区域点云
        std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())->PublishUncoveredCloud();
        // 发布未覆盖边界点云
        std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())->PublishUncoveredFrontierCloud();
    }
    ...
    void SensorCoveragePlanner3D::UpdateGlobalRepresentation()
    {
        ...
        Eigen::Vector3d pointcloud_manager_neighbor_cells_origin =
            std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())->GetPointCloudManagerNeighborCellsOrigin();
        ...
        {
            std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())->SetUseFrontier(false);
        }
        std::dynamic_pointer_cast<planning_env_ns::PlanningEnv>(pd_.GetPlanningEnv())->UpdateKeyposeCloud<PlannerCloudPointType>(pd_.GetKeyposeCloud()->cloud_);

        int closest_node_ind =
            std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->GetClosestNodeInd(pd_.GetRobotPosition()); // 获取当前机器人位置最近的keypose节点索引
        geometry_msgs::Point closest_node_position = std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())->GetClosestNodePosition(
            pd_.GetRobotPosition()); // 获取当前机器人位置最近的keypose节点的位置
        std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->SetCurKeyposeGraphNodeInd(closest_node_ind); // grid_world_设置当前keypose节点索引和位置
        std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->SetCurKeyposeGraphNodePosition(closest_node_position); // grid_world_设置当前keypose节点位置

        std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->UpdateRobotPosition(pd_.GetRobotPosition()); // 更新机器人位置在grid_world_
        if (!std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->HomeSet())
        {
            std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->SetHomePosition(pd_.GetInitialPosition());
                                                 ...
                                                 exploration_path_ns::ExplorationPath& global_path)
    {
        ...
        std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->UpdateCellStatus(std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())); // 更新cell状态
        std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->UpdateCellKeyposeGraphNodes(std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())); // 更新keypose_graph
        std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->AddPathsInBetweenCells(std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager()), std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())); // 搜寻两两cell之间的路径

        std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->UpdateCandidateViewPointCellStatus(std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())); // 根据候选视点更新cell状态

        // 边框Viewpoint
        global_path = std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->SolveGlobalTSP(std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager()), global_cell_tsp_order,
                                                      std::dynamic_pointer_cast<keypose_graph_ns::KeyposeGraph>(pd_.GetKeyposeGraph())); // 由视点、keypose_graph求解全局路径
    ...
    void SensorCoveragePlanner3D::execute(const ros::TimerEvent&)
    {
        ...
        if (keypose_cloud_update_)
        {
            ...
            {
                std::dynamic_pointer_cast<viewpoint_manager_ns::ViewPointManager>(pd_.GetViewpointManager())->ResetViewPointCoverage();
            }
            ...
            if (std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld())->IsReturningHome() && std::dynamic_pointer_cast<local_coverage_planner_ns::LocalCoveragePlanner>(pd_.GetLocalCoveragePlanner())->IsLocalCoverageComplete() &&
            ...
            pd_.GetVisualizer()->GetGlobalSubspaceMarker(std::dynamic_pointer_cast<grid_world_ns::GridWorld>(pd_.GetGridWorld()), global_cell_tsp_order);
