...
namespace grid_world_ns
{
        ...
        path_added_to_keypose_graph_(false), roadmap_connection_point_set_(false)
    {
        ...
        robot_position_.y = 0;
        robot_position_.z = 0;
        status_ = grid_world_ns::CellStatus::UNSEEN;
    }
    ...
    GridWorld::GridWorld(const int row_num, const int col_num, const int level_num, const double cell_size,
        ...
        initialized_(false), use_keypose_graph_(false), cur_keypose_(0, 0, 0), cur_robot_cell_ind_(-1), prev_robot_cell_ind_(-1)
    {
        robot_position_.x = 0.0;
        robot_position_.y = 0.0;
        robot_position_.z = 0.0;
    ...
    void GridWorld::ReadParameters(ros::NodeHandle& nh)
    {
        ...
        const auto viewpoint_resolution = misc_utils_ns::getParam<double>(nh, "viewpoint_manager/resolution_x", 1.0);
        // 视点采样间距？
    ...
    void
    GridWorld::UpdateCellKeyposeGraphNodes(const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph) const
    {
        ...
        {
            const geometry_msgs::Point node_position = keypose_graph->GetNodePosition(node_ind); // 获取节点位置
            const int cell_ind =
                GetCellInd(node_position.x, node_position.y, node_position.z); // 获取该节点位置所对应的cell索引
            if (subspaces_->InRange(cell_ind))
            {
                if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::EXPLORING)
                {
    ...
    void GridWorld::GetMarker(visualization_msgs::Marker& marker) const
    {
        ...
        for (int i = 0; i < kRowNum; i++)
        {
            for (int j = 0; j < kColNum; j++)
            {
                for (int k = 0; k < kLevelNum; k++)
                {
                    ...
                    if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::UNSEEN)
                    {
                        color.r = 0.0;
                        color.g = 0.0;
                        color.b = 1.0;
                    ...
                    else if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::COVERED)
                    {
                        color.r = 1.0;
                        color.g = 1.0;
                        color.b = 0.0;
                    ...
                    else if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::EXPLORING)
                    {
                        color.r = 0.0;
                        color.g = 1.0;
                        color.b = 0.0;
                    ...
                    else if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::NOGO)
                    {
                        color.r = 1.0;
                        color.g = 0.0;
                        color.b = 0.0;
        ...
        //     if(cells_[ind].GetStatus() == grid_world_ns::CellStatus::UNSEEN) continue;
    ...
    void GridWorld::GetVisualizationCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& vis_cloud) const
    {
        ...
        for (int i = 0; i < subspaces_->GetCellNumber(); i++)
        {
            grid_world_ns::CellStatus cell_status = subspaces_->GetCell(i).GetStatus();
    ...
    // 获取GridWorld中所有处于探索状态（grid_world_ns::CellStatus::EXPLORING）的网格单元的索引，
    ...
    grid_world_ns::CellStatus GridWorld::Getgrid_world_ns::CellStatus(const int cell_ind) const
    {
        MY_ASSERT(subspaces_->InRange(cell_ind));
        return subspaces_->GetCell(cell_ind).GetStatus();
    }

    void GridWorld::Setgrid_world_ns::CellStatus(const int cell_ind, const grid_world_ns::CellStatus status) const
    {
        MY_ASSERT(subspaces_->InRange(cell_ind));
    ...
    int GridWorld::Getgrid_world_ns::CellStatusCount(const grid_world_ns::CellStatus status) const
    {
        int count = 0;
        for (int i = 0; i < subspaces_->GetCellNumber(); i++)
        {
    ...
    void GridWorld::getSemiExploredCellIndices(const std::vector<Eigen::Vector3d>& semi_dynamic_frontier_positions)
    {
        ...
        for (const auto& position : semi_dynamic_frontier_positions)
        {
            ...
            {
                subspaces_->GetCell(index).SetStatus(grid_world_ns::CellStatus::SEMI_EXPLORED);
                semi_explored_cell_indices_.push_back(index);
            }
        }
    }

    void GridWorld::Updategrid_world_ns::CellStatus(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager)
    {
        int exploring_count = 0;
        int unseen_count = 0;
        int covered_count = 0;
        ...
        for (int i = 0; i < subspaces_->GetCellNumber(); ++i)
        {
            if (subspaces_->GetCell(i).GetStatus() == grid_world_ns::CellStatus::EXPLORING) // exploring
            {
                exploring_count++;
            }
            else if (subspaces_->GetCell(i).GetStatus() == grid_world_ns::CellStatus::UNSEEN) // unexplored
            {
                unseen_count++;
            }
            else if (subspaces_->GetCell(i).GetStatus() == grid_world_ns::CellStatus::COVERED) // explored
            {
                covered_count++;
            }
            else if (subspaces_->GetCell(i).GetStatus() == grid_world_ns::CellStatus::SEMI_EXPLORED) // semi-explored
            {
                semi_explored_count++;
            }
        }
        ...
        for (const auto& cell_ind : neighbor_cell_indices_)
        {
            if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::COVERED_BY_OTHERS)
            {
                continue;
            }
            ...
            if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::EXPLORING &&
                ...
                candidate_count > 0)
            {
                subspaces_->GetCell(cell_ind).SetStatus(grid_world_ns::CellStatus::COVERED); //
            }
            ...
            {
                subspaces_->GetCell(cell_ind).SetStatus(grid_world_ns::CellStatus::EXPLORING);
            ...
            // Unseen to Exploring
            else if (subspaces_->GetCell(cell_ind).GetStatus() != grid_world_ns::CellStatus::COVERED && selected_viewpoint_count > 0)
            {
                subspaces_->GetCell(cell_ind).SetStatus(grid_world_ns::CellStatus::EXPLORING);
            ...
            else if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::EXPLORING && candidate_count == 0)
            {
                // First visit
                ...
                else
                {
                    ...
                    if (xy_dist_to_robot < kCellSize && z_dist_to_robot < kCellHeight * 0.8)
                    {
                        subspaces_->GetCell(cell_ind).SetStatus(
                            grid_world_ns::CellStatus::COVERED); // 如果该cell在机器人附近，则设置为COVERED
                    }
                }
            }
            // 当局部区域存在semi-explored子空间时，将其转化为exploring状态
            else if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::SEMI_EXPLORED) // todo
            {
                subspaces_->GetCell(cell_ind).SetStatus(grid_world_ns::CellStatus::EXPLORING);
            }

            // 如果该cell处于Exploring状态，并且有候选视点，则设置该cell的robot position和keypose id
            if (subspaces_->GetCell(cell_ind).GetStatus() == grid_world_ns::CellStatus::EXPLORING && candidate_count > 0)
            {
    ...
    GridWorld::AddPathsInBetweenCells(const std::shared_ptr<viewpoint_manager_ns::ViewPointManager>& viewpoint_manager,
                                      const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph) const
    {
    ...
    // 判断路径是否有效？
    bool GridWorld::PathValid(const nav_msgs::Path& path, const int from_cell_ind, const int to_cell_ind) const
    {
    ...
    bool
    GridWorld::HasDirectKeyposeGraphConnection(const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph,
