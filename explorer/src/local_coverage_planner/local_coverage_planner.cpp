...
int LocalCoveragePlanner::GetBoundaryViewpointIndex(const exploration_path_ns::ExplorationPath& global_path)
{
  ...
  {
    if (viewpoint_manager_->InLocalPlanningHorizon(global_path.nodes_.front().position_))// 如果全局路径的第一个节点在局部规划窗口内
    {
      ...
      for (int i = 0; i < global_path.nodes_.size(); i++)
      {
        ...
        boundary_viewpoint_index = viewpoint_manager_->GetNearestCandidateViewPointInd(global_path.nodes_[i].position_);// 以离该点最近的候选视点作为边界视点
      }
    }
  }
                                                         ...
                                                         std::vector<int>& navigation_viewpoint_indices)
{
  ...
  robot_viewpoint_ind_ = viewpoint_manager_->GetNearestCandidateViewPointInd(robot_position_);// 获取离机器人当前位置最近的候选视点的索引作为机器人视点
  lookahead_viewpoint_ind_ = viewpoint_manager_->GetNearestCandidateViewPointInd(lookahead_point_);// 同上，获取离lookahead_point_最近的候选视点视点的索引
  
  if (!lookahead_point_update_ || !viewpoint_manager_->InRange(lookahead_viewpoint_ind_))
  {
    lookahead_viewpoint_ind_ = robot_viewpoint_ind_;// 如果lookahead_point_没有更新或者不在local planning range内，则lookahead_viewpoint_ind_ = robot_viewpoint_ind_
  }
  // Get connecting viewpoints to the global path
  GetBoundaryViewpointIndices(global_path);// 获取边界viewpoint索引，两个边界视点
                                                      ...
                                                      const std::vector<int>& selected_viewpoint_array_indices)
{
  ...
  for (const auto& viewpoint_index : viewpoint_manager_->GetViewPointCandidateIndices())
  {
    ...
    {
      cover_point_queue.emplace_back(covered_point_num, viewpoint_index);//在队列末尾插入一个pair，第一个元素是覆盖点数，第二个元素是对应的视点索引
    }
    ...
    { 
      ...
      {
        frontier_queue.emplace_back(covered_frontier_point_num, viewpoint_index);// 在队列末尾插入一个pair，第一个元素是覆盖frontier点数，第二个元素是对应的视点索引
      }
    }
  }
                                           ...
                                           std::vector<int>& selected_viewpoint_indices, bool use_frontier)
{
  ...
  while (true)
  {
    ...
    {
      int add_point_num = 0;
      int ind = queue_copy[i].second;// 按顺序
    ...
    const exploration_path_ns::ExplorationPath& global_path, int uncovered_point_num, int uncovered_frontier_point_num)
{
  ...
  GetNavigationViewPointIndices(global_path, navigation_viewpoint_indices);// 返回四个viewpoint(两个边界点、lookahead point、robot_viewpoint)
  ...
  for (const auto& ind : reused_viewpoint_indices)
  {
    ...
    pre_selected_viewpoint_array_indices.push_back(viewpoint_array_ind);// 将该viewpoint对应的索引加入到预选的viewpoint索引队列中
  }
  ...
  for (const auto& ind : navigation_viewpoint_indices)
  {
    ...
    pre_selected_viewpoint_array_indices.push_back(array_ind);// 将该导航视点的索引加入到预选的viewpoint索引队列中
  }
  ...
  std::vector<std::pair<int, int>> frontier_queue;
  // 对所有的候选视点进行排序(降序排序)，根据covered的point和frontier进行，其中去除访问过的候选视点、预选的视点、不在Exploring cell中的候选视点
  // 未对covered和frontier_covered进行改变
