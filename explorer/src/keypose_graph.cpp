...
namespace keypose_graph_ns
{
    ...
    KeyposeGraph::KeyposeGraph(ros::NodeHandle& nh) :
        allow_vertical_edge_(false), current_keypose_id_(0), kAddNodeMinDist(1.0),
        kAddEdgeCollisionCheckResolution(0.4), kAddEdgeCollisionCheckRadius(0.3), kAddEdgeConnectDistThr(3.0),
        kAddEdgeToLastKeyposeDistThr(3.0), kAddEdgeVerticalThreshold(1.0), kAddEdgeCollisionCheckPointNumThr(1)
    {
        ReadParameters(nh);
    ...
    void KeyposeGraph::AddNodeAndEdge(const geometry_msgs::Point& position, int node_ind, int keypose_id,
    ...
    bool KeyposeGraph::HasNode(const Eigen::Vector3d& position)
    {
        ...
        {
            double xy_dist =
                misc_utils_ns::PointXYDist<geometry_msgs::Point>(geo_position, nodes_[closest_node_ind].position_);
    ...
    void KeyposeGraph::GetMarker(visualization_msgs::Marker& node_marker, visualization_msgs::Marker& edge_marker)
    {
        ...
        for (int i = 0; i < graph_.size(); i++)
        {
            ...
            for (int j = 0; j < graph_[i].size(); j++)
            {
                ...
                if (std::find(added_edge.begin(), added_edge.end(), std::make_pair(start_ind, end_ind)) ==
                    added_edge.end())
                {
    ...
    void
    KeyposeGraph::CheckLocalCollision(const geometry_msgs::Point& robot_position,
                ...
                else
                {
                    ...
                    for (int j = 0; j < graph_[i].size(); j++)
                    {
                        ...
                        Eigen::Vector3d end_position =
                            Eigen::Vector3d(nodes_[neighbor_ind].position_.x, nodes_[neighbor_ind].position_.y,
            ...
            if (min_dist_ind >= 0 && min_dist_ind < nodes_.size())
            {
                if (min_dist > kAddNodeMinDist)
                {
                        ...
                        last_keypose_ind < nodes_.size())
                    {
                        ...
                        AddNodeAndEdge(current_keypose_position_, new_node_ind, current_keypose_id_, true,
                                       last_keypose_ind, last_keypose_dist);
                    }
                    else
                    {
                    ...
                    if (!in_range_node_indices.empty())
                    {
                        for (int idx = 0; idx < in_range_node_indices.size(); idx++)
                        {
                            ...
                            if (in_range_ind >= 0 && in_range_ind < nodes_.size())
                            {
                                ...
                                int check_point_num =
                                    static_cast<int>(neighbor_node_dist / kAddEdgeCollisionCheckResolution);
                                ...
                                for (int i = 0; i < check_point_num; i++)
                                {
                                    ...
                                    double check_point_x = current_keypose_position_.x +
                                        kAddEdgeCollisionCheckResolution * i * diff_x / neighbor_node_dist;
                                    double check_point_y = current_keypose_position_.y +
                                        kAddEdgeCollisionCheckResolution * i * diff_y / neighbor_node_dist;
                                    double check_point_z = current_keypose_position_.z +
                                        kAddEdgeCollisionCheckResolution * i * diff_z / neighbor_node_dist;
    ...
    // 获取离该位置最近的节点的索引和距离
    void KeyposeGraph::GetClosestConnectedNodeIndAndDistance(const geometry_msgs::Point& point, int& node_ind,
                                                             double& dist)
    {
        ...
        {
            node_ind =
                static_cast<int>(connected_nodes_cloud_->points[nearest_neighbor_node_indices.front()].intensity);
                                                    ...
                                                    bool get_path, nav_msgs::Path& path)
    {
        ...
        for (int i = 0; i < nodes_.size(); i++)
        {
            if (allow_vertical_edge_)
            {
                ...
                double dist_to_target = misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(
            ...
            else
            {
                ...
                {
                    double xy_dist_to_start = misc_utils_ns::PointXYDist<geometry_msgs::Point, geometry_msgs::Point>(
                        nodes_[i].position_, start_point);
                ...
                {
                    double xy_dist_to_target = misc_utils_ns::PointXYDist<geometry_msgs::Point, geometry_msgs::Point>(
                        nodes_[i].position_, target_point);
        ...
        bool found_path = misc_utils_ns::AStarSearchWithMaxPathLength(
            graph_, dist_, node_positions, from_idx, to_idx, get_path, path_indices, shortest_dist, max_path_length);
    ...
    double KeyposeGraph::GetShortestPath(const geometry_msgs::Point& start_point,
                                         const geometry_msgs::Point& target_point, bool get_path, nav_msgs::Path& path,
                                         bool use_connected_nodes) const
    {
        if (nodes_.size() < 2)
        {
            if (get_path)
    ...
    geometry_msgs::Point KeyposeGraph::GetNodePosition(const int node_ind) const
    {
        ...
        {
            ROS_WARN_STREAM("KeyposeGraph::GetNodePosition: node_ind " << node_ind << " out of bound [0, "
                                                                       << nodes_.size() - 1 << "]");
        }
