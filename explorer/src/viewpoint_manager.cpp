...
namespace viewpoint_manager_ns
{
                ...
                for (int z = 0; z < vp_.kNumber.z(); z++)
                {
                    ...
                    for (auto& i : idx_addon)
                    {
                        ...
                        if (grid_->InRange(neighbor_sub))
                        {
                            ...
                            double dist = sqrt(vp_.kResolution.x() * vp_.kResolution.x() * std::abs(i.x()) +
    ...
    void ViewPointManager::GetCollisionCorrespondence()
    {
        ...
        collision_grid_ =
            std::make_unique<grid_ns::Grid<std::vector<int>>>(vp_.kCollisionGridSize, viewpoint_index_correspondence,
                                                              collision_grid_origin_, vp_.kCollisionGridResolution, 2);
        ...
        for (int x = 0; x < vp_.kCollisionGridSize.x(); x++)
        {
            for (int y = 0; y < vp_.kCollisionGridSize.y(); y++)
            {
                for (int z = 0; z < vp_.kCollisionGridSize.z(); z++)
                {
                    ...
                    {
                        int viewpoint_ind = static_cast<int>(viewpoint_cloud->points[ind].intensity);
    ...
    bool ViewPointManager::UpdateRobotPosition(const Eigen::Vector3d& robot_position)
    {
        ...
        {
            robot_grid_sub(i) =
                diff(i) > 0 ? static_cast<int>(diff(i) / (vp_.kRolloverStepsize(i) * vp_.kResolution(i))) : -1;
        }
    ...
    int ViewPointManager::GetViewPointInd(int viewpoint_array_ind) const { return grid_->GetInd(viewpoint_array_ind); }
    ...
    void ViewPointManager::GetVisualizationCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& vis_cloud)
    {
        vis_cloud->clear();
        for (int i = 0; i < vp_.kViewPointNumber; i++)
        {
    ...
    void ViewPointManager::CheckViewPointLineOfSight()
    {
        ...
        std::vector<Eigen::Vector3i> ray_cast_cells;
        const Eigen::Vector3i max_sub(vp_.kNumber.x() - 1, vp_.kNumber.y() - 1, vp_.kNumber.z() - 1);
    ...
    bool ViewPointManager::InFOV(const Eigen::Vector3d& point_position, const Eigen::Vector3d& viewpoint_position) const
    {
    ...
    // Collision
    bool ViewPointManager::ViewPointInCollision(const int viewpoint_ind, const bool use_array_ind) const
    {
    ...
    // Line of Sight
    bool ViewPointManager::ViewPointInLineOfSight(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].InLineOfSight();
    }
    void ViewPointManager::SetViewPointInLineOfSight(const int viewpoint_ind, const bool in_line_of_sight,
                                                     const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // Connectivity
    bool ViewPointManager::ViewPointConnected(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // Visited
    bool ViewPointManager::ViewPointVisited(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].Visited();
    }
    void ViewPointManager::SetViewPointVisited(const int viewpoint_ind, const bool visited, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetVisited(visited);
    }
    // Selected
    bool ViewPointManager::ViewPointSelected(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].Selected();
    }
    void ViewPointManager::SetViewPointSelected(const int viewpoint_ind, const bool selected, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetSelected(selected);
    }
    // Candidacy
    bool ViewPointManager::IsViewPointCandidate(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // Terrain Height
    bool ViewPointManager::ViewPointHasTerrainHeight(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // In exploring cell
    bool ViewPointManager::ViewPointInExploringCell(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // Height
    double ViewPointManager::GetViewPointHeight(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetHeight();
    }
    void ViewPointManager::SetViewPointHeight(const int viewpoint_ind, const double height, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetHeight(height);
    }
    // In current frame line of sight
    bool ViewPointManager::ViewPointInCurrentFrameLineOfSight(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // Position
    geometry_msgs::Point ViewPointManager::GetViewPointPosition(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // Cell Ind
    int ViewPointManager::GetViewPointCellInd(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCellInd();
    }
    void ViewPointManager::SetViewPointCellInd(const int viewpoint_ind, const int cell_ind, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetCellInd(cell_ind);
    }
    // Collision frame count
    int ViewPointManager::GetViewPointCollisionFrameCount(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCollisionFrameCount();
    }
    void ViewPointManager::AddViewPointCollisionFrameCount(const int viewpoint_ind, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].AddCollisionFrame();
    }
    void ViewPointManager::ResetViewPointCollisionFrameCount(const int viewpoint_ind, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].ResetCollisionFrameCount();
    }
    // Covered point list
    void ViewPointManager::ResetViewPointCoveredPointList(const int viewpoint_ind, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].ResetCoveredPointList();
        viewpoints_[array_ind].ResetCoveredFrontierPointList();
    }
    void ViewPointManager::AddUncoveredPoint(const int viewpoint_ind, const int point_ind, const bool use_array_ind)
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    const std::vector<int>& ViewPointManager::GetViewPointCoveredFrontierPointList(const int viewpoint_ind,
                                                                                   const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    int ViewPointManager::GetViewPointCoveredPointNum(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCoveredPointNum();
    }

    int ViewPointManager::GetViewPointCoveredFrontierPointNum(const int viewpoint_ind, const bool use_array_ind) const
    {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCoveredFrontierPointNum();
    }

    int ViewPointManager::GetViewPointCoveredPointNum(const std::vector<bool>& point_list, const int viewpoint_index,
                                                      const bool use_array_ind) const
    {
    ...
    void ViewPointManager::UpdateViewPointCoveredPoint(std::vector<bool>& point_list, const int viewpoint_index,
                                                       const bool use_array_ind) const
    {
        for (const auto& point_ind : GetViewPointCoveredPointList(viewpoint_index, use_array_ind))
        {
    ...
    void
    ViewPointManager::UpdateCandidateViewPointCellStatus(std::unique_ptr<grid_world_ns::GridWorld> const& grid_world)
    {
        for (const auto& ind : candidate_indices_)
        {
            ...
            {
                ROS_WARN_STREAM("ViewPointManager::UpdateCandidateViewPointCellStatus: cell ind " << cell_ind
                                                                                                  << " out of bound");
            }
        }
    }
    ...
    int ViewPointManager::GetNearestCandidateViewPointInd(const Eigen::Vector3d& position)
    {
        ...
        if (!candidate_indices_.empty())
        {
            ...
            for (const auto& cur_viewpoint_ind : candidate_indices_)
            {
                ...
                double dist = misc_utils_ns::PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(cur_position,
                                                                                                      query_position);
                if (dist < min_dist)
                {
                    min_dist = dist;
    ...
    void ViewPointManager::GetCollisionViewPointVisCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr& cloud) const
    {
        cloud->clear();
    ...
    bool ViewPointManager::InLocalPlanningHorizon(const Eigen::Vector3d& position)
    {
        ...
        if (InRange(viewpoint_ind))
        {
            const double max_z_diff = std::max(vp_.kResolution.x(), vp_.kResolution.y()) * 2; // 最大高度差为2倍分辨率
