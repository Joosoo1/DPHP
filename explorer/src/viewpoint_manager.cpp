...
namespace viewpoint_manager_ns {
    bool ViewPointManagerParameter::ReadParameters(ros::NodeHandle& nh) {
        ...
        kResolution.x() =
            misc_utils_ns::getParam<double>(nh, "viewpoint_manager/resolution_x", 0.5);
        kResolution.y() =
            misc_utils_ns::getParam<double>(nh, "viewpoint_manager/resolution_y", 0.5);
        kResolution.z() =
            misc_utils_ns::getParam<double>(nh, "viewpoint_manager/resolution_z", 0.5);
        kConnectivityHeightDiffThr =
            misc_utils_ns::getParam<double>(nh, "kConnectivityHeightDiffThr", 0.25);
        kViewPointCollisionMargin =
            misc_utils_ns::getParam<double>(nh, "kViewPointCollisionMargin", 0.5);
        kViewPointCollisionMarginZPlus =
            misc_utils_ns::getParam<double>(nh, "kViewPointCollisionMarginZPlus", 0.5);
        kViewPointCollisionMarginZMinus =
            misc_utils_ns::getParam<double>(nh, "kViewPointCollisionMarginZMinus", 0.5);
        kCollisionGridZScale = misc_utils_ns::getParam<double>(nh, "kCollisionGridZScale", 2.0);
        kCollisionGridResolution.x() =
            misc_utils_ns::getParam<double>(nh, "kCollisionGridResolutionX", 0.5);
        kCollisionGridResolution.y() =
            misc_utils_ns::getParam<double>(nh, "kCollisionGridResolutionY", 0.5);
        kCollisionGridResolution.z() =
            misc_utils_ns::getParam<double>(nh, "kCollisionGridResolutionZ", 0.5);
        kLineOfSightStopAtNearestObstacle =
            misc_utils_ns::getParam<bool>(nh, "kLineOfSightStopAtNearestObstacle", true);
        kCheckDynamicObstacleCollision =
            misc_utils_ns::getParam<bool>(nh, "kCheckDynamicObstacleCollision", true);
        kCollisionFrameCountMax = misc_utils_ns::getParam<int>(nh, "kCollisionFrameCountMax", 3);
        kViewPointHeightFromTerrain =
            misc_utils_ns::getParam<double>(nh, "kViewPointHeightFromTerrain", 0.75);
        ...
        for (int i = 0; i < dimension_; i++) {
            kCollisionGridSize(i) =
                ceil((kNumber(i) * kResolution(i) + kViewPointCollisionMargin * 2)
                     / kCollisionGridResolution(i));
        }

        kCoverageOcclusionThr = misc_utils_ns::getParam<double>(nh, "kCoverageOcclusionThr", 1.0);
        kCoverageDilationRadius =
            misc_utils_ns::getParam<double>(nh, "kCoverageDilationRadius", 1.0);
        kCoveragePointCloudResolution =
            misc_utils_ns::getParam<double>(nh, "kSurfaceCloudDwzLeafSize", 1.0);
    ...
    ViewPointManager::ViewPointManager(ros::NodeHandle& nh) : initialized_(false) {
        ...
        kdtree_viewpoint_candidate_ = boost::make_shared<pcl::KdTreeFLANN<pcl::PointXYZI>>();
        kdtree_viewpoint_in_collision_ = boost::make_shared<pcl::KdTreeFLANN<pcl::PointXYZI>>();
        ...
        for (int x = 0; x < vp_.kNumber.x(); x++) {
            for (int y = 0; y < vp_.kNumber.y(); y++) {
                for (int z = 0; z < vp_.kNumber.z(); z++) {
                    const Eigen::Vector3i sub(x, y, z);
                ...
                for (int z = 0; z < vp_.kNumber.z(); z++) {
                    ...
                    for (auto& i : idx_addon) {
                        ...
                        if (grid_->InRange(neighbor_sub)) {
                            connected_neighbor_indices_[ind].push_back(
                                grid_->Sub2Ind(neighbor_sub));
                            double dist =
                                sqrt(vp_.kResolution.x() * vp_.kResolution.x() * std::abs(i.x())
                                     + vp_.kResolution.y() * vp_.kResolution.y() * std::abs(i.y())
                                     + vp_.kResolution.z() * vp_.kResolution.z() * std::abs(i.z()));
    ...
    void ViewPointManager::ComputeInRangeNeighborIndices() {
        ...
        const auto kdtree = boost::make_shared<pcl::KdTreeFLANN<pcl::PointXYZ>>();
    ...
    void ViewPointManager::GetCollisionCorrespondence() {
        ...
        collision_grid_ = std::make_unique<grid_ns::Grid<std::vector<int>>>(
            vp_.kCollisionGridSize, viewpoint_index_correspondence, collision_grid_origin_,
            vp_.kCollisionGridResolution, 2);
        ...
        for (int x = 0; x < vp_.kNumber.x(); x++) {
            for (int y = 0; y < vp_.kNumber.y(); y++) {
                for (int z = 0; z < vp_.kNumber.z(); z++) {
                    const int ind = grid_->Sub2Ind(Eigen::Vector3i(x, y, z));
                    pcl::PointXYZI point;
        ...
        // std::cout << "computing collision grid viewpoint cloud size: " << viewpoint_cloud->points.size() <<
        // std::endl;
        ...
        for (int x = 0; x < vp_.kCollisionGridSize.x(); x++) {
            for (int y = 0; y < vp_.kCollisionGridSize.y(); y++) {
                for (int z = 0; z < vp_.kCollisionGridSize.z(); z++) {
                    ...
                    kdtree->radiusSearch(query_point, vp_.kViewPointCollisionMargin,
                                         nearby_viewpoint_indices, nearby_viewpoint_sqdist);
                    const int grid_ind = collision_grid_->Sub2Ind(x, y, z);
                    for (const int ind : nearby_viewpoint_indices) {
                        int viewpoint_ind =
                            static_cast<int>(viewpoint_cloud->points[ind].intensity);
    ...
    bool ViewPointManager::UpdateRobotPosition(const Eigen::Vector3d& robot_position) {
            ...
            for (int x = 0; x < vp_.kNumber.x(); x++) {
                for (int y = 0; y < vp_.kNumber.y(); y++) {
                    for (int z = 0; z < vp_.kNumber.z(); z++) {
                        ...
                        position.x =
                            origin_.x() + x * vp_.kResolution.x() + vp_.kResolution.x() / 2.0;
                        position.y =
                            origin_.y() + y * vp_.kResolution.y() + vp_.kResolution.y() / 2.0;
        ...
        for (int i = 0; i < vp_.dimension_; i++) {
            robot_grid_sub(i) = diff(i) > 0
                ? static_cast<int>(diff(i) / (vp_.kRolloverStepsize(i) * vp_.kResolution(i)))
                : -1;
        }
    ...
    Eigen::Vector3i ViewPointManager::GetViewPointSub(const Eigen::Vector3d& position) {
    ...
    void ViewPointManager::GetVisualizationCloud(
        ...
        for (int i = 0; i < vp_.kViewPointNumber; i++) {
            if (IsViewPointCandidate(i, true)) {
                const geometry_msgs::Point position = GetViewPointPosition(i, true);
    ...
    void ViewPointManager::CheckViewPointCollisionWithCollisionGrid(
        ...
        for (const auto& point : collision_cloud->points) {
            Eigen::Vector3i collision_grid_sub =
                collision_grid_->Pos2Sub(point.x, point.y, point.z);
            if (collision_grid_->InRange(collision_grid_sub)) {
                ...
                if (collision_point_count_[collision_grid_ind] >= vp_.kCollisionPointThr) {
                    std::vector<int> collision_viewpoint_indices =
                        collision_grid_->GetCellValue(collision_grid_ind);
                    for (const int viewpoint_ind : collision_viewpoint_indices) {
                        MY_ASSERT(viewpoint_ind >= 0 && viewpoint_ind < vp_.kViewPointNumber);
                        const double z_diff = point.z - GetViewPointHeight(viewpoint_ind);
    ...
    void ViewPointManager::CheckViewPointCollision(
        const pcl::PointCloud<pcl::PointXYZI>::Ptr& collision_cloud) {
        CheckViewPointCollisionWithCollisionGrid(collision_cloud);
        CheckViewPointBoundaryCollision();
    }

    void ViewPointManager::CheckViewPointCollisionWithTerrain(
    ...
    void ViewPointManager::CheckViewPointLineOfSightHelper(const Eigen::Vector3i& start_sub,
                                                           const Eigen::Vector3i& end_sub,
                                                           const Eigen::Vector3i& max_sub,
                                                           const Eigen::Vector3i& min_sub) {
        if (end_sub == start_sub) return;
        const int viewpoint_ind = grid_->Sub2Ind(end_sub);
        ...
        if (ray_cast_cells.size() > 1) {
            if (vp_.kLineOfSightStopAtNearestObstacle) {
                ...
                for (int i = 1; i < ray_cast_cells.size(); i++) {
                    const int ind = grid_->Sub2Ind(ray_cast_cells[i]);
    ...
    void ViewPointManager::CheckViewPointLineOfSight() {
        ...
        const Eigen::Vector3i robot_sub = GetViewPointSub(robot_position_);
        MY_ASSERT(grid_->InRange(robot_sub));
        const int robot_viewpoint_ind = grid_->Sub2Ind(robot_sub);
        ...
        std::vector<Eigen::Vector3i> ray_cast_cells;
        const Eigen::Vector3i max_sub(vp_.kNumber.x() - 1, vp_.kNumber.y() - 1,
                                      vp_.kNumber.z() - 1);
        const Eigen::Vector3i min_sub(0, 0, 0);

        int x_indices[2] = {0, vp_.kNumber.x() - 1};
        const int y_indices[2] = {0, vp_.kNumber.y() - 1};
        const int z_indices[2] = {0, vp_.kNumber.z() - 1};
        ...
        for (int x = 0; x < vp_.kNumber.x(); x++) {
            for (int y : y_indices) {
                for (int z = 0; z < vp_.kNumber.z(); z++) {
                    Eigen::Vector3i end_sub(x, y, z);
                    const int array_ind = grid_->GetArrayInd(end_sub);
                    if (!checked[array_ind]) {
                        CheckViewPointLineOfSightHelper(robot_sub, end_sub, max_sub, min_sub);
                        checked[array_ind] = true;
                    }
    ...
    void ViewPointManager::CheckViewPointInFOV() {
        ...
        for (int i = 0; i < vp_.kViewPointNumber; i++) {
            geometry_msgs::Point viewpoint_position = GetViewPointPosition(i, true);
            if (!InRobotFOV(Eigen::Vector3d(viewpoint_position.x, viewpoint_position.y,
    ...
    bool ViewPointManager::InFOV(const Eigen::Vector3d& point_position,
    ...
    void ViewPointManager::CheckViewPointConnectivity() {
        ...
        if (ViewPointInCollision(robot_ind)) {
            ...
            for (int i = 0; i < vp_.kViewPointNumber; i++) {
                const int array_ind = grid_->GetArrayInd(i);
                if (!ViewPointInCollision(i)) {
                    ...
                    const double dist_to_robot = (viewpoint_position - robot_position_).norm();
            ...
            if (!found_collision_free_viewpoint) {
                std::cout << "All viewpoints in collision, existing" << std::endl;
                return;
            }
        }
        ...
        while (!queue.empty()) {
            ...
            for (int i = 0; i < connected_neighbor_indices_[cur_ind].size(); i++) {
                ...
                if (!grid_->InRange(neighbor_ind)) {
                    std::cout
                        << "ViewPointManager::CheckViewPointConnectivity: neighbor ind out of bound"
                        << std::endl;
                    continue;
                }
                if (!checked[neighbor_ind] && !ViewPointInCollision(neighbor_ind)
                    && ViewPointInLineOfSight(neighbor_ind)) {
                    if (std::abs(GetViewPointHeight(cur_ind) - GetViewPointHeight(neighbor_ind))
                        < vp_.kConnectivityHeightDiffThr) {
    ...
    void ViewPointManager::UpdateViewPointVisited(const std::vector<Eigen::Vector3d>& positions) {
        ...
        for (const auto& position : positions) {
            ...
            if (grid_->InRange(viewpoint_sub)) {
                ...
                for (const auto& neighbor_viewpoint_ind :
    ...
    void ViewPointManager::UpdateViewPointVisited(
        std::unique_ptr<grid_world_ns::GridWorld> const& grid_world) {
        for (int i = 0; i < viewpoints_.size(); i++) {
            const geometry_msgs::Point viewpoint_position = GetViewPointPosition(i, true);
            const int cell_ind = grid_world->GetCellInd(viewpoint_position.x, viewpoint_position.y,
                                                        viewpoint_position.z);
    ...
    void ViewPointManager::SetViewPointHeightWithTerrain(
        const pcl::PointCloud<pcl::PointXYZI>::Ptr& terrain_cloud,
        double terrain_height_threshold) {
        // Set the height of the viewpoint nearby the robot to be the height of the robot, in case there is no terrain
        // cloud within the blind spot.
        if (!initialized_) return;
        const Eigen::Vector3i robot_sub = GetViewPointSub(robot_position_);
        const int robot_ind = grid_->Sub2Ind(robot_sub);
        MY_ASSERT(grid_->InRange(robot_sub));
        geometry_msgs::Point robot_viewpoint_position = GetViewPointPosition(robot_ind);

        if (!ViewPointHasTerrainHeight(robot_ind)
            || std::abs(robot_viewpoint_position.z - robot_position_.z())
            ...
            for (int neighbor_ind : in_range_neighbor_indices_[robot_ind]) {
        ...
        for (const auto& terrain_point : terrain_cloud->points) {
            ...
            if (grid_->InRange(viewpoint_sub)) {
                const int viewpoint_ind = grid_->Sub2Ind(viewpoint_sub);
                const double target_height = terrain_point.z + vp_.kViewPointHeightFromTerrain;
                // If the viewpoint has not been set height with terrain points, or if there is a terrain point with a
                // lower height
                if (!ViewPointHasTerrainHeight(viewpoint_ind)
                    || target_height < GetViewPointHeight(viewpoint_ind)) {
            ...
            if (!ViewPointHasTerrainHeight(i)) {
                for (const auto& neighbor_ind : in_range_neighbor_indices_[i]) {
                    ...
                    if (ViewPointHasTerrainHeight(neighbor_ind)) {
                        ...
                        if (std::abs(neighbor_height - GetViewPointHeight(i))
                            > vp_.kViewPointHeightFromTerrainChangeThreshold) {
    ...
    // Reset viewpoint
    void ViewPointManager::ResetViewPoint(const int viewpoint_ind, const bool use_array_ind) {
    ...
    // Collision
    bool ViewPointManager::ViewPointInCollision(const int viewpoint_ind,
    ...
    void ViewPointManager::SetViewPointCollision(const int viewpoint_ind, const bool in_collision,
                                                 const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetInCollision(in_collision);
    }
    // Line of Sight
    bool ViewPointManager::ViewPointInLineOfSight(const int viewpoint_ind,
                                                  const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].InLineOfSight();
    }
    void ViewPointManager::SetViewPointInLineOfSight(const int viewpoint_ind,
                                                     const bool in_line_of_sight,
                                                     const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetInLineOfSight(in_line_of_sight);
    }
    // Connectivity
    bool ViewPointManager::ViewPointConnected(const int viewpoint_ind,
                                              const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].Connected();
    }
    void ViewPointManager::SetViewPointConnected(const int viewpoint_ind, const bool connected,
                                                 const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetConnected(connected);
    }
    // Visited
    bool ViewPointManager::ViewPointVisited(const int viewpoint_ind,
                                            const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].Visited();
    }
    void ViewPointManager::SetViewPointVisited(const int viewpoint_ind, const bool visited,
                                               const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetVisited(visited);
    }
    // Selected
    bool ViewPointManager::ViewPointSelected(const int viewpoint_ind,
                                             const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].Selected();
    }
    void ViewPointManager::SetViewPointSelected(const int viewpoint_ind, const bool selected,
                                                const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetSelected(selected);
    }
    // Candidacy
    bool ViewPointManager::IsViewPointCandidate(const int viewpoint_ind,
                                                const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // Terrain Height
    bool ViewPointManager::ViewPointHasTerrainHeight(const int viewpoint_ind,
                                                     const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].HasTerrainHeight();
    }
    void ViewPointManager::SetViewPointHasTerrainHeight(const int viewpoint_ind,
    ...
    // In exploring cell
    bool ViewPointManager::ViewPointInExploringCell(const int viewpoint_ind,
                                                    const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].InExploringCell();
    }
    void ViewPointManager::SetViewPointInExploringCell(const int viewpoint_ind,
                                                       const bool in_exploring_cell,
                                                       const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetInExploringCell(in_exploring_cell);
    }
    // Height
    double ViewPointManager::GetViewPointHeight(const int viewpoint_ind,
                                                const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetHeight();
    }
    void ViewPointManager::SetViewPointHeight(const int viewpoint_ind, const double height,
                                              const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetHeight(height);
    }
    // In current frame line of sight
    bool ViewPointManager::ViewPointInCurrentFrameLineOfSight(const int viewpoint_ind,
                                                              const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].InCurrentFrameLineOfSight();
    }
    void ViewPointManager::SetViewPointInCurrentFrameLineOfSight(
        const int viewpoint_ind, const bool in_current_frame_line_of_sight,
        const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetInCurrentFrameLineOfSight(in_current_frame_line_of_sight);
    }
    // Position
    geometry_msgs::Point ViewPointManager::GetViewPointPosition(const int viewpoint_ind,
                                                                const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetPosition();
    }
    void ViewPointManager::SetViewPointPosition(const int viewpoint_ind,
    ...
    // Cell Ind
    int ViewPointManager::GetViewPointCellInd(const int viewpoint_ind,
                                              const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCellInd();
    }
    void ViewPointManager::SetViewPointCellInd(const int viewpoint_ind, const int cell_ind,
                                               const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].SetCellInd(cell_ind);
    }
    // Collision frame count
    int ViewPointManager::GetViewPointCollisionFrameCount(const int viewpoint_ind,
                                                          const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCollisionFrameCount();
    }
    void ViewPointManager::AddViewPointCollisionFrameCount(const int viewpoint_ind,
                                                           const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].AddCollisionFrame();
    }
    void ViewPointManager::ResetViewPointCollisionFrameCount(const int viewpoint_ind,
                                                             const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].ResetCollisionFrameCount();
    }
    // Covered point list
    void ViewPointManager::ResetViewPointCoveredPointList(const int viewpoint_ind,
                                                          const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        viewpoints_[array_ind].ResetCoveredPointList();
        viewpoints_[array_ind].ResetCoveredFrontierPointList();
    }
    void ViewPointManager::AddUncoveredPoint(const int viewpoint_ind, const int point_ind,
                                             const bool use_array_ind) {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
    ...
    // 获取该视点的覆盖点列表
    const std::vector<int>& ViewPointManager::GetViewPointCoveredPointList(
        const int viewpoint_ind, const bool use_array_ind) const {
        // 获取视点数组索引
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCoveredPointList();
    }
    const std::vector<int>& ViewPointManager::GetViewPointCoveredFrontierPointList(
        const int viewpoint_ind, const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCoveredFrontierPointList();
    }

    int ViewPointManager::GetViewPointCoveredPointNum(const int viewpoint_ind,
                                                      const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCoveredPointNum();
    }

    int ViewPointManager::GetViewPointCoveredFrontierPointNum(const int viewpoint_ind,
                                                              const bool use_array_ind) const {
        const int array_ind = GetViewPointArrayInd(viewpoint_ind, use_array_ind);
        return viewpoints_[array_ind].GetCoveredFrontierPointNum();
    }

    int ViewPointManager::GetViewPointCoveredPointNum(const std::vector<bool>& point_list,
    ...
    int ViewPointManager::GetViewPointCoveredFrontierPointNum(
        const std::vector<bool>& frontier_point_list, const int viewpoint_index,
        const bool use_array_ind) const {
    ...
    void ViewPointManager::UpdateViewPointCoveredPoint(std::vector<bool>& point_list,
                                                       const int viewpoint_index,
                                                       const bool use_array_ind) const {
        for (const auto& point_ind : GetViewPointCoveredPointList(viewpoint_index, use_array_ind)) {
            MY_ASSERT(misc_utils_ns::InRange<bool>(point_list, point_ind));
    ...
    void ViewPointManager::UpdateViewPointCoveredFrontierPoint(
    ...
    int ViewPointManager::GetViewPointCandidate() {
        ...
        GetCandidateViewPointGraph(candidate_viewpoint_graph_, candidate_viewpoint_dist_,
    ...
    nav_msgs::Path ViewPointManager::GetViewPointShortestPath(int start_viewpoint_ind,
        ...
        if (!InRange(start_viewpoint_ind)) {
            ROS_WARN_STREAM("ViewPointManager::GetViewPointShortestPath start viewpoint ind: "
                            << start_viewpoint_ind << " not in range");
            return path;
        }
        if (!InRange(target_viewpoint_ind)) {
            ROS_WARN_STREAM("ViewPointManager::GetViewPointShortestPath target viewpoint ind: "
                            << target_viewpoint_ind << " not in range");
            return path;
        }
        ...
        misc_utils_ns::AStarSearch(candidate_viewpoint_graph_, candidate_viewpoint_dist_,
                                   candidate_viewpoint_position_, start_graph_ind, target_graph_ind,
                                   true, path_graph_indices);
    ...
    nav_msgs::Path ViewPointManager::GetViewPointShortestPath(
        const Eigen::Vector3d& start_position, const Eigen::Vector3d& target_position) {
        nav_msgs::Path path;
        if (!InLocalPlanningHorizon(start_position)) {
            ROS_WARN_STREAM("ViewPointManager::GetViewPointShortestPath start position "
                            << start_position.transpose() << " not in local planning horizon");
            return path;
        }
        if (!InLocalPlanningHorizon(target_position)) {
            ROS_WARN_STREAM("ViewPointManager::GetViewPointShortestPath target position "
    ...
    bool ViewPointManager::GetViewPointShortestPathWithMaxLength(
        const Eigen::Vector3d& start_position, const Eigen::Vector3d& target_position,
        ...
        bool found_path = misc_utils_ns::AStarSearchWithMaxPathLength(
            candidate_viewpoint_graph_, candidate_viewpoint_dist_, candidate_viewpoint_position_,
            start_graph_ind, target_graph_ind, true, path_graph_indices, shortest_path_dist,
            max_path_length);
    ...
    void ViewPointManager::UpdateCandidateViewPointCellStatus(
        std::unique_ptr<grid_world_ns::GridWorld> const& grid_world) {
        for (const auto& ind : candidate_indices_) {
            ...
            if (grid_world->IndInBound(cell_ind)) {
                ...
                if (cell_status == grid_world_ns::CellStatus::UNSEEN
                ...
                ROS_WARN_STREAM("ViewPointManager::UpdateCandidateViewPointCellStatus: cell ind "
                                << cell_ind << " out of bound");
            }
        }
    }

    void ViewPointManager::GetCandidateViewPointGraph(
        std::vector<std::vector<int>>& graph, std::vector<std::vector<double>>& dist,
        ...
        for (int i = 0; i < candidate_indices_.size(); i++) {
            const int ind = candidate_indices_[i];
            graph_index_map_[ind] = i;
        }

        // Build the graph
        for (int i = 0; i < candidate_indices_.size(); i++) {
            const int cur_ind = candidate_indices_[i];
            positions.push_back(GetViewPointPosition(cur_ind));
            for (int j = 0; j < connected_neighbor_indices_[cur_ind].size(); j++) {
                const int neighbor_ind = connected_neighbor_indices_[cur_ind][j];
    ...
    int ViewPointManager::GetNearestCandidateViewPointInd(const Eigen::Vector3d& position) {
        ...
        std::cout
            << "Candidate viewpoint empty, can't find nearest candidate viewpoints to the position"
            << std::endl;
        return -1;
    }

    void ViewPointManager::GetCollisionViewPointVisCloud(
    ...
    bool ViewPointManager::InLocalPlanningHorizon(const Eigen::Vector3d& position) {
        ...
        if (InRange(viewpoint_ind)) {
            const double max_z_diff =
                std::max(vp_.kResolution.x(), vp_.kResolution.y()) * 2;  // 最大高度差为2倍分辨率
            const geometry_msgs::Point viewpoint_position =
                GetViewPointPosition(viewpoint_ind);  // 获取该点对应的Viewpoint的位置
