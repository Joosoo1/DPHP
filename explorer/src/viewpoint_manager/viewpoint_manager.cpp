...
bool ViewPointManager::InLocalPlanningHorizon(const Eigen::Vector3d& position)
{
  ...
  if (InRange(viewpoint_ind))
  {
    double max_z_diff = std::max(vp_.kResolution.x(), vp_.kResolution.y()) * 2;//最大高度差为2倍分辨率
    geometry_msgs::Point viewpoint_position = GetViewPointPosition(viewpoint_ind);// 获取该点对应的Viewpoint的位置
