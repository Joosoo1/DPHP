...
namespace sensor_coverage_planner_3d_ns {
    bool PlannerParameters::ReadParameters(ros::NodeHandle& nh) {
        sub_start_exploration_topic_ = misc_utils_ns::getParam<std::string>(
            nh, "sub_start_exploration_topic_", "/exploration_start");
        sub_state_estimation_topic_ = misc_utils_ns::getParam<std::string>(
            nh, "sub_state_estimation_topic_", "/state_estimation_at_scan");
        sub_registered_scan_topic_ = misc_utils_ns::getParam<std::string>(
            nh, "sub_registered_scan_topic_", "/registered_scan");
        sub_nogo_boundary_topic_ =
            misc_utils_ns::getParam<std::string>(nh, "sub_nogo_boundary_topic_", "/nogo_boundary");
        pub_exploration_finish_topic_ = misc_utils_ns::getParam<std::string>(
            nh, "pub_exploration_finish_topic_", "exploration_finish");
        pub_runtime_breakdown_topic_ = misc_utils_ns::getParam<std::string>(
            nh, "pub_runtime_breakdown_topic_", "runtime_breakdown");
        pub_runtime_topic_ =
            misc_utils_ns::getParam<std::string>(nh, "pub_runtime_topic_", "/runtime");
        pub_waypoint_topic_ =
            misc_utils_ns::getParam<std::string>(nh, "pub_waypoint_topic_", "/way_point");
        ...
        kUseLineOfSightLookAheadPoint =
            misc_utils_ns::getParam<bool>(nh, "kUseLineOfSightLookAheadPoint", true);
        kNoExplorationReturnHome =
            misc_utils_ns::getParam<bool>(nh, "kNoExplorationReturnHome", true);
        kUseMomentum = misc_utils_ns::getParam<bool>(nh, "kUseMomentum", false);

        // Double
        kKeyposeCloudDwzFilterLeafSize =
            misc_utils_ns::getParam<double>(nh, "kKeyposeCloudDwzFilterLeafSize", 0.2);
        ...
        kTerrainCollisionThreshold =
            misc_utils_ns::getParam<double>(nh, "kTerrainCollisionThreshold", 0.5);
        kLookAheadDistance = misc_utils_ns::getParam<double>(nh, "kLookAheadDistance", 5.0);
        kExtendWayPointDistanceBig =
            misc_utils_ns::getParam<double>(nh, "kExtendWayPointDistanceBig", 8.0);
        kExtendWayPointDistanceSmall =
            misc_utils_ns::getParam<double>(nh, "kExtendWayPointDistanceSmall", 3.0);

        // Int
        kDirectionChangeCounterThr =
            misc_utils_ns::getParam<int>(nh, "kDirectionChangeCounterThr", 4);
        kDirectionNoChangeCounterThr =
            misc_utils_ns::getParam<int>(nh, "kDirectionNoChangeCounterThr", 5);
    ...
    void PlannerData::Initialize(ros::NodeHandle& nh, ros::NodeHandle& nh_p) {
        ...
        terrain_ext_collision_cloud_ =
            std::make_unique<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>(
        ...
        local_coverage_planner_ =
            std::make_unique<local_coverage_planner_ns::LocalCoveragePlanner>(nh_p);
        ...
        nogo_boundary_marker_ =
            std::make_unique<misc_utils_ns::Marker>(nh, "nogo_boundary_marker", kWorldFrameID);
        ...
        // grid_world子空间可视化
        grid_world_marker_ =
            std::make_unique<misc_utils_ns::Marker>(nh, "grid_world_marker", kWorldFrameID);
        ...
        const double add_non_keypose_node_min_dist =
    ...
    bool SensorCoveragePlanner3D::initialize(ros::NodeHandle& nh, ros::NodeHandle& nh_p) {
        ...
        lidar_model_ns::LiDARModel::setCloudDWZResol(
            pd_.planning_env_->GetPlannerCloudResolution());

        // 以1hz的频率执行探索
        execution_timer_ =
            nh.createTimer(ros::Duration(0.1), &SensorCoveragePlanner3D::execute, this);

        // 是否开始探索的话题
        exploration_start_sub_ = nh.subscribe<std_msgs::Bool>(
            pp_.sub_start_exploration_topic_, 5, [&](const std_msgs::Bool::ConstPtr& start_msg) {
                if (start_msg->data) { start_exploration_ = true; }
            });

        // 订阅配准之后的点云->世界坐标系下的点云
        registered_scan_sub_ = nh.subscribe(pp_.sub_registered_scan_topic_, 5,
                                            &SensorCoveragePlanner3D::RegisteredScanCallback, this);
        // 订阅里程计的状态估计信息
        state_estimation_sub_ = nh.subscribe<nav_msgs::Odometry>(
            pp_.sub_state_estimation_topic_, 5, &SensorCoveragePlanner3D::StateEstimationCallback,
            this);
        // 订阅划定的探索范围(need fix)
        nogo_boundary_sub_ = nh.subscribe<geometry_msgs::PolygonStamped>(
            pp_.sub_nogo_boundary_topic_, 5, &SensorCoveragePlanner3D::NogoBoundaryCallback, this);
        // 订阅半动态物体
        semi_dynamic_objects_sub_ = nh.subscribe<geometry_msgs::PoseStamped>(
            "/trans_system/door_positions", 10,
            &SensorCoveragePlanner3D::SemiDynamicObjectsCallback, this);
        // 获取范围内的动态障碍物及其预测状态(速度)
        dynamic_obstacles_client_ = nh.serviceClient<onboard_detector::GetDynamicObstacles>(
        ...
        to_nearest_global_subspace_path_publisher_ =
            nh.advertise<nav_msgs::Path>("to_nearest_global_subspace_path", 1);
    ...
    void SensorCoveragePlanner3D::RegisteredScanCallback(
        const sensor_msgs::PointCloud2ConstPtr& registered_cloud_msg) {
        // 等待接受里程计信息，完成初始化
        if (!initialized_) { return; }
        pcl::PointCloud<pcl::PointXYZ>::Ptr registered_scan_tmp(
            new pcl::PointCloud<pcl::PointXYZ>());
        pcl::fromROSMsg(*registered_cloud_msg, *registered_scan_tmp);
        // 如果接收到点云为空，则直接返回
        ...
        *(pd_.registered_scan_stack_->cloud_) += *(registered_scan_tmp);  // 将点云加入点云堆栈中
        ...
        pointcloud_downsizer_.Downsize(registered_scan_tmp, pp_.kKeyposeCloudDwzFilterLeafSize,
                                       pp_.kKeyposeCloudDwzFilterLeafSize,
                                       pp_.kKeyposeCloudDwzFilterLeafSize);
        // 清空扫描点云
        ...
        if (registered_cloud_count_ == 0)  // 每过5帧标记一帧为关键帧
        {
            ...
            pd_.cur_keypose_node_ind_ =
                pd_.keypose_graph_->AddKeyposeNode(pd_.keypose_, *(pd_.planning_env_));

            // 对配准后的点云堆栈进行下采样
            pointcloud_downsizer_.Downsize(
                pd_.registered_scan_stack_->cloud_, pp_.kKeyposeCloudDwzFilterLeafSize,
                pp_.kKeyposeCloudDwzFilterLeafSize, pp_.kKeyposeCloudDwzFilterLeafSize);

            pd_.keypose_cloud_->cloud_->clear();  // 清空当前关键帧点云
            pcl::copyPointCloud(
                *(pd_.registered_scan_stack_->cloud_),
    ...
    void SensorCoveragePlanner3D::TerrainMapCallback(
        const sensor_msgs::PointCloud2ConstPtr& terrain_map_msg) const {
        if (pp_.kCheckTerrainCollision) {
            const pcl::PointCloud<pcl::PointXYZI>::Ptr terrain_map_tmp(
            ...
            for (auto& point : terrain_map_tmp->points) {
                if (point.intensity > pp_.kTerrainCollisionThreshold) {
                    pd_.terrain_collision_cloud_->cloud_->points.push_back(
                        point);  // 提取地形图中的障碍物点云
                }
            }
        }
    ...
    void SensorCoveragePlanner3D::SendInitialWaypoint() const {
        // send waypoint ahead
        constexpr double lx = 12.0;
        constexpr double ly = 0.0;
        const double dx = cos(pd_.robot_yaw_) * lx - sin(pd_.robot_yaw_) * ly;
        const double dy = sin(pd_.robot_yaw_) * lx + cos(pd_.robot_yaw_) * ly;
        ...
        std::cout << "init waypoint is: " << waypoint.pose.position.x << ", "
    ...
    void SensorCoveragePlanner3D::UpdateKeyposeGraph() const {
        ...
        pd_.keypose_graph_->GetMarker(pd_.keypose_graph_node_marker_->marker_,
                                      pd_.keypose_graph_edge_marker_->marker_);
    ...
    int SensorCoveragePlanner3D::UpdateViewPoints() {
        ...
        const int viewpoint_candidate_count =
            pd_.viewpoint_manager_->GetViewPointCandidate();  // 获取候选视点的数量

        UpdateVisitedPositions();  // 更新无人车已经访问过的位置
        pd_.viewpoint_manager_->UpdateViewPointVisited(
            pd_.visited_positions_);  // 更新视点是否被访问过
        pd_.viewpoint_manager_->UpdateViewPointVisited(
            pd_.grid_world_);  // 更新栅格地图中的cell是否被访问过
    ...
    void SensorCoveragePlanner3D::UpdateCoveredAreas(int& uncovered_point_num,
                                                     int& uncovered_frontier_point_num) const {
        // Update covered area
        misc_utils_ns::Timer update_coverage_area_timer("update covered area");
        update_coverage_area_timer.Start();
        pd_.planning_env_->UpdateCoveredArea(pd_.robot_viewpoint_,
                                             pd_.viewpoint_manager_);  // 更新已覆盖区域
        ...
        pd_.planning_env_->GetUncoveredArea(pd_.viewpoint_manager_, uncovered_point_num,
    ...
    void SensorCoveragePlanner3D::UpdateVisitedPositions() {
        ...
        for (int i = 0; i < pd_.visited_positions_.size(); i++) {
            constexpr double kVisitedPositionThreshold = 1.0;
    ...
    void SensorCoveragePlanner3D::UpdateGlobalRepresentation() const {
        ...
        pd_.planning_env_->UpdateRobotPosition(
            pd_.robot_position_);  // planning_env_中更新机器人位置
        // 没有......
        ...
        pd_.grid_world_->SetCurKeyposeGraphNodePosition(
            closest_node_position);  // grid_world_设置当前keypose节点位置
    ...
    void SensorCoveragePlanner3D::GlobalPlanning(
        ...
        pd_.viewpoint_manager_->UpdateCandidateViewPointCellStatus(
            pd_.grid_world_);  // 根据候选视点更新cell状态

        // 边框Viewpoint
        global_path = pd_.grid_world_->SolveGlobalTSP(
            pd_.viewpoint_manager_, global_cell_tsp_order,
    ...
    void SensorCoveragePlanner3D::LocalPlanning(
        const int uncovered_point_num, const int uncovered_frontier_point_num,
        const exploration_path_ns::ExplorationPath& global_path,
        ...
        local_path = pd_.local_coverage_planner_->SolveLocalCoverageProblem(
    ...
    void SensorCoveragePlanner3D::PublishLocalPlanningVisualization(
        ...
        pd_.local_coverage_planner_->GetSelectedViewPointVisCloud(
            pd_.selected_viewpoint_vis_cloud_->cloud_);
    ...
    exploration_path_ns::ExplorationPath SensorCoveragePlanner3D::ConcatenateGlobalLocalPath(
        ...
        // 获取当前全局和局部路径的长度
        const double global_path_length = global_path.GetLength();
        const double local_path_length = local_path.GetLength();
    ...
    bool SensorCoveragePlanner3D::GetLookAheadPoint(
        ...
        Eigen::Vector3d robot_position(pd_.robot_position_.x, pd_.robot_position_.y,
                                       pd_.robot_position_.z);
        ...
        */
        ...
        for (int i = robot_i + 1; i < local_path.GetNodeNum(); i++) {
            length_from_robot +=
                (local_path.nodes_[i].position_ - local_path.nodes_[i - 1].position_).norm();
        ...
        if (forward_viewpoint_count > 0 && !has_forward) {
            std::cout << "forward viewpoint count > 0 but does not have forward lookahead point"
                      << std::endl;
            exit(1);
        }
        if (backward_viewpoint_count > 0 && !has_backward) {
            std::cout << "backward viewpoint count > 0 but does not have backward lookahead point"
                      << std::endl;
            exit(1);
        }
        ...
        if (has_forward) {
            ...
            forward_angle_score =
                dx * forward_diff.x() + dy * forward_diff.y();  // 与前向lookahead点方向的夹角
        }
        if (has_backward) {
            Eigen::Vector3d backward_diff = backward_lookahead_point - robot_position;
            backward_diff.z() = 0.0;
            backward_diff = backward_diff.normalized();
            backward_angle_score =
                dx * backward_diff.x() + dy * backward_diff.y();  // 与前向lookahead点方向的夹角
        }
        if (has_lookahead) {
        ...
        if (relocation_)  // 该区域无Viewpoint时
        {
            ...
            } else  // 如果不使用惯性动量，则根据距离，选取短距离的lookahead_point
            {
                ...
                if (dist_from_start < dist_from_end
                 ...
                 && dist_robot_to_lookahead > pp_.kLookAheadDistance / 2
                 && pd_.viewpoint_manager_->InLocalPlanningHorizon(
    ...
    void SensorCoveragePlanner3D::PublishWaypoint() {
        ...
        if (exploration_finished_ && near_home_ && pp_.kRushHome) {
            ...
            double extend_dist = lookahead_point_in_line_of_sight_
                ? pp_.kExtendWayPointDistanceBig
                : pp_.kExtendWayPointDistanceSmall;
    ...
    void SensorCoveragePlanner3D::PublishRuntime() {
        local_viewpoint_sampling_runtime_ =
            pd_.local_coverage_planner_->GetViewPointSamplingRuntime() / 1000;
        local_path_finding_runtime_ = (pd_.local_coverage_planner_->GetFindPathRuntime()
                                       + pd_.local_coverage_planner_->GetTSPRuntime())
            / 1000;
    ...
    double SensorCoveragePlanner3D::GetRobotToHomeDistance() const {
        const Eigen::Vector3d robot_position(pd_.robot_position_.x, pd_.robot_position_.y,
                                             pd_.robot_position_.z);
    ...
    //
    void SensorCoveragePlanner3D::PrintExplorationStatus(const std::string& status,
                                                         const bool clear_last_line) {
        if (clear_last_line) {
            printf(cursup);
            printf(cursclean);
            printf(cursup);
    ...
    void SensorCoveragePlanner3D::SemiDynamicObjectsCallback(
        ...
        pd_.cur_semi_obstacles_pose_.push_back(*msg);
    }

    void SensorCoveragePlanner3D::getLocalDynamicObstacles(
        const geometry_msgs::Point& robot_position, const double& range) {
        onboard_detector::GetDynamicObstacles srv;
        srv.request.current_position.x = robot_position.x;
        srv.request.current_position.y = robot_position.y;
        srv.request.current_position.z = robot_position.z;
        ...
        if (dynamic_obstacles_client_.call(srv)) {
            ...
            for (int i = 0; i < srv.response.position.size(); ++i) {
                ...
                dynamic_box3d.x = srv.response.position[i].x;
                dynamic_box3d.y = srv.response.position[i].y;
                dynamic_box3d.z = srv.response.position[i].z;
                dynamic_box3d.Vx = srv.response.velocity[i].x;
                dynamic_box3d.Vy = srv.response.velocity[i].y;
                dynamic_box3d.Vz = srv.response.velocity[i].z;
                dynamic_box3d.x_width = srv.response.size[i].x;
                dynamic_box3d.y_width = srv.response.size[i].y;
                dynamic_box3d.z_width = srv.response.size[i].z;
    ...
    void SensorCoveragePlanner3D::execute(const ros::TimerEvent&) {
        ...
        if (keypose_cloud_update_) {
            ...
            LocalPlanning(uncovered_point_num, uncovered_frontier_point_num, global_path,
                          local_path);
            ...
            if (pd_.grid_world_->IsReturningHome()
                && pd_.local_coverage_planner_->IsLocalCoverageComplete()
            ...
            lookahead_point_update_ =
                GetLookAheadPoint(pd_.exploration_path_, global_path, pd_.lookahead_point_);
            PublishWaypoint();
            ...
            pd_.visualizer_->GetLocalPlanningHorizonMarker(
                viewpoint_origin.x(), viewpoint_origin.y(), pd_.robot_position_.z);
