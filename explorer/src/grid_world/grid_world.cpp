    ...
    for (int i = 0; i < subspaces_->GetCellNumber(); i++)
    {
      ...
      if (subspaces_->GetCell(i).GetStatus() == CellStatus::EXPLORING)
      {
            ...
            (subspaces_->GetCell(i).GetViewPointIndices().empty() && subspaces_->GetCell(i).GetVisitCount() > 1))
        {
          ...
          {
            Eigen::Vector3d connection_point = subspaces_->GetCell(i).GetRoadmapConnectionPoint(); // 获取连接点
            ...
            if (reachable)
            {
              ...
              exploring_cell_indices.push_back(i); // 将连接点加入exploring_cell_positions和exploring_cell_indices中
            }
          }
        }
      }
    ...
    for (int i = 0; i < neighbor_cell_indices_.size(); i++)
    {
      ...
      for (int x = -1; x <= 1; x++)
      {
        for (int y = -1; y <= 1; y++)
        {
          for (int z = -1; z <= 1; z++)
          {
            if (std::abs(x) + std::abs(y) + std::abs(z) == 1) // 栅格空间内与from_cell_ind相邻的cell(6个),上下左右前后，距离为一个单元格cell
            {
      ...
      for (int j = 0; j < nearby_cell_indices.size(); j++)
      {
        ...
        if (subspaces_->GetCell(to_cell_ind).GetViewPointIndices().empty()) // 如果该相邻的cell内无viewpoint，则跳过该cell
        {
          continue;
        }
