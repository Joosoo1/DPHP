...
void PlannerData::Initialize(ros::NodeHandle& nh, ros::NodeHandle& nh_p)
{
  ...
  local_coverage_planner_->SetViewPointManager(viewpoint_manager_);// local_coverage_planner与viewpoint_manager关联，用于局部规划
  ...
  keypose_graph_->SetAddNonKeyposeNodeMinDist() = add_non_keypose_node_min_dist;// 设置非关键节点最小距离，即普通点
...
void SensorCoveragePlanner3D::RegisteredScanCallback(const sensor_msgs::PointCloud2ConstPtr& registered_scan_msg)
{
  ...
  *(pd_.registered_scan_stack_->cloud_) += *(registered_scan_tmp);// 将点云加入点云堆栈中 
  ...
  if (registered_cloud_count_ == 0)// 每过5帧标记一帧为关键帧
  {
    ...
    pcl::copyPointCloud(*(pd_.registered_scan_stack_->cloud_), *(pd_.keypose_cloud_->cloud_));// 将点云堆栈中的点云赋值给当前关键帧点云
...
void SensorCoveragePlanner3D::TerrainMapExtCallback(const sensor_msgs::PointCloud2ConstPtr& terrain_map_ext_msg)
{
  ...
  if (pp_.kCheckTerrainCollision)
  {
    pcl::fromROSMsg<pcl::PointXYZI>(*terrain_map_ext_msg, *(pd_.large_terrain_cloud_->cloud_));// 获取扩展地形图
...
void SensorCoveragePlanner3D::SendInitialWaypoint()
{
  ...
  std::cout << "init waypoint is: " << waypoint.pose.position.x << ", " << waypoint.pose.position.y << ", " << waypoint.pose.position.z << std::endl;
...
int SensorCoveragePlanner3D::UpdateViewPoints()
{
  ...
  if (pp_.kUseTerrainHeight)
  {
    pd_.viewpoint_manager_->SetViewPointHeightWithTerrain(pd_.large_terrain_cloud_->cloud_);// 根据扩展地形图点云获取视点的高度
  }
  ...
  pd_.viewpoint_manager_->CheckViewPointCollision(pd_.collision_cloud_->cloud_);// 输入障碍物点云，检查视点是否与障碍物碰撞
  ...
  pd_.viewpoint_manager_->GetCollisionViewPointVisCloud(pd_.viewpoint_in_collision_cloud_->cloud_);// 获取存在碰撞的视点点云
...
void SensorCoveragePlanner3D::UpdateViewPointCoverage()
{
  ...
  pd_.viewpoint_manager_->UpdateViewPointCoverage<PlannerCloudPointType>(pd_.planning_env_->GetDiffCloud());// 输入新点云，更新视点的覆盖率
...
void SensorCoveragePlanner3D::UpdateGlobalRepresentation()
{
  pd_.local_coverage_planner_->SetRobotPosition(
      Eigen::Vector3d(pd_.robot_position_.x, pd_.robot_position_.y, pd_.robot_position_.z));// 在局部规划器中，更新机器人位置

  bool viewpoint_rollover = pd_.viewpoint_manager_->UpdateRobotPosition(
      Eigen::Vector3d(pd_.robot_position_.x, pd_.robot_position_.y, pd_.robot_position_.z));// 更新机器人位置，并判断是否发生了回绕？
  ...
  // 没有......
  pd_.planning_env_->GetVisualizationPointCloud(pd_.point_cloud_manager_neighbor_cloud_->cloud_);// 获取点云管理器中邻近cell的点云存储到point_cloud_manager_neighbor_cloud_
  ...
  int closest_node_ind = pd_.keypose_graph_->GetClosestNodeInd(pd_.robot_position_);// 获取当前机器人位置最近的keypose节点索引
  geometry_msgs::Point closest_node_position = pd_.keypose_graph_->GetClosestNodePosition(pd_.robot_position_);// 获取当前机器人位置最近的keypose节点的位置
                                             ...
                                             exploration_path_ns::ExplorationPath& global_path)
{
  ...
  // 边框Viewpoint
  global_path = pd_.grid_world_->SolveGlobalTSP(pd_.viewpoint_manager_, global_cell_tsp_order, pd_.keypose_graph_);// 由视点、keypose_graph求解全局路径
                                                ...
                                                Eigen::Vector3d& lookahead_point)
{
  ...
  */ 
  ...
  for (int i = robot_i + 1; i < local_path.GetNodeNum(); i++)
  {
    ...
    {
      in_line_of_sight = pd_.viewpoint_manager_->InCurrentFrameLineOfSight(local_path.nodes_[i + 1].position_);// 判断是否在当前帧的视场内
    }
  ...
  {
    if (use_momentum_ && pp_.kUseMomentum)// 如果使用惯性动量，避免频繁切换，则根据前向、后向lookahead点分数选择lookahead_point
    {
...
void SensorCoveragePlanner3D::PublishWaypoint()
{
  ...
  double x_diff = waypoint.pose.position.x - pd_.robot_position_.x;
  double y_diff = waypoint.pose.position.y - pd_.robot_position_.y;
...
Eigen::Quaterniond SensorCoveragePlanner3D::GetQuaternionFromYaw(double yaw)
{
  Eigen::Quaterniond q;
