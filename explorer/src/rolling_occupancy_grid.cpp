...
namespace rolling_occupancy_grid_ns {
    RollingOccupancyGrid::RollingOccupancyGrid(ros::NodeHandle& nh)
        : initialized_(false), dimension_(3) {
        const auto pointcloud_cell_size =
            misc_utils_ns::getParam<double>(nh, "kPointCloudCellSize", 18);
        const auto pointcloud_cell_height =
            misc_utils_ns::getParam<double>(nh, "kPointCloudCellHeight", 1.8);
        const int pointcloud_cell_neighbor_number =
        ...
        resolution_.x() =
            misc_utils_ns::getParam<double>(nh, "rolling_occupancy_grid/resolution_x", 0.3);
        resolution_.y() =
            misc_utils_ns::getParam<double>(nh, "rolling_occupancy_grid/resolution_y", 0.3);
        resolution_.z() =
            misc_utils_ns::getParam<double>(nh, "rolling_occupancy_grid/resolution_z", 0.3);
        ...
        occupancy_array_ =
            std::make_unique<grid_ns::Grid<CellState>>(grid_size_, UNKNOWN, origin_, resolution_);
    ...
    bool RollingOccupancyGrid::UpdateRobotPosition(const Eigen::Vector3d& robot_position) {
        ...
        for (int i = 0; i < dimension_; i++) {
            robot_grid_sub(i) = diff(i) > 0
                ? static_cast<int>(diff(i) / (rollover_step_size_(i) * resolution_(i)))
                : -1;
        }
        ...
        for (int i = 0; i < dimension_; i++) {
            rollover_step(i) = std::abs(sub_diff(i)) > 0
                ? rollover_step_size_(i) * ((sub_diff(i) > 0) ? 1 : -1) * std::abs(sub_diff(i))
                : 0;
        }
        ...
        for (const auto& ind : rolled_out_grid_indices) {
            ...
            } else if (occupancy_array_->GetCellValue(array_ind) == OCCUPIED) {
                Eigen::Vector3d position = occupancy_array_->Ind2Pos(ind);
                pcl::PointXYZI point;
                point.x = position.x();
                point.y = position.y();
        ...
        for (const auto& ind : updated_grid_indices) {
            occupancy_array_->SetCellValue(ind, UNKNOWN);
        }
        ...
        for (const auto& ind : updated_grid_indices_) {
            if (occupancy_array_->InRange(ind)) {
                ...
                if (!occupancy_array_->InRange(cur_sub)) {
                    ROS_WARN_STREAM("RollingOccupancyGrid::RayTrace() " << cur_sub.transpose()
                                                                        << " sub out of range");
                    continue;
                }
                int array_ind = rolling_grid_->GetArrayInd(ind);
    ...
    void RollingOccupancyGrid::RayTraceHelper(const Eigen::Vector3i& start_sub,
        ...
        double t_max_x =
            step_x == 0 ? DBL_MAX : misc_utils_ns::intbound(start_sub.x(), diff_sub.x());
        double t_max_y =
            step_y == 0 ? DBL_MAX : misc_utils_ns::intbound(start_sub.y(), diff_sub.y());
        double t_max_z =
            step_z == 0 ? DBL_MAX : misc_utils_ns::intbound(start_sub.z(), diff_sub.z());
        ...
        const double t_delta_z =
            step_z == 0 ? DBL_MAX : static_cast<double>(step_z) / static_cast<double>(diff_sub.z());
        double dist = 0;
        ...
        while (occupancy_array_->InRange(cur_sub)) {
            ...
            if (cur_sub == end_sub || dist > max_dist
                || occupancy_array_->GetCellValue(array_ind) == OCCUPIED) {
                return;
            }
            if (t_max_x < t_max_y) {
                if (t_max_x < t_max_z) {
    ...
    void RollingOccupancyGrid::GetFrontier(
        ...
        const Eigen::Vector3i origin_sub = occupancy_array_->Pos2Sub(origin);
    ...
    void RollingOccupancyGrid::GetVisualizationCloud(
        const pcl::PointCloud<pcl::PointXYZI>::Ptr& vis_cloud) const {
        vis_cloud->clear();
        const int cell_number = occupancy_array_->GetCellNumber();
        for (int i = 0; i < cell_number; i++) {
            int array_ind = rolling_grid_->GetArrayInd(i);
            if (occupancy_array_->GetCellValue(array_ind) == OCCUPIED
                || occupancy_array_->GetCellValue(array_ind) == FREE) {
                ...
                if (occupancy_array_->GetCellValue(array_ind) == OCCUPIED) {
                    point.intensity = 0.0;
                } else if (occupancy_array_->GetCellValue(array_ind) == FREE) {
                    point.intensity = 1.0;
                } else {
                    point.intensity = 2.0;
                }
