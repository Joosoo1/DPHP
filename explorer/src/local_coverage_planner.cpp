...
namespace local_coverage_planner_ns {
    ...
    bool LocalCoveragePlannerParameter::ReadParameters(ros::NodeHandle& nh) {
        ...
        kGreedyViewPointSampleRange =
            misc_utils_ns::getParam<int>(nh, "kGreedyViewPointSampleRange", 5);
        kLocalPathOptimizationItrMax =
            misc_utils_ns::getParam<int>(nh, "kLocalPathOptimizationItrMax", 10);
        return true;
    }
    ...
    int LocalCoveragePlanner::GetBoundaryViewpointIndex(
        const exploration_path_ns::ExplorationPath& global_path) const {
        int boundary_viewpoint_index =
            robot_viewpoint_ind_;  // 初始化为机器人当前位置最接近的视点索引
        if (!global_path.nodes_.empty())  // 如果全局路径不为空
        {
            if (viewpoint_manager_->InLocalPlanningHorizon(
                    global_path.nodes_.front()
                        .position_))  // 如果全局路径的第一个节点在局部规划窗口内
            {
    ...
    void LocalCoveragePlanner::GetBoundaryViewpointIndices(
        ...
        start_viewpoint_ind_ =
            GetBoundaryViewpointIndex(global_path);  // 顺向与局部规划范围相交为起始视点
        global_path.Reverse();
        end_viewpoint_ind_ =
            GetBoundaryViewpointIndex(global_path);  // 逆向与局部规划范围相交为结束视点
    }
    ...
    // 更新视点的覆盖point
    void LocalCoveragePlanner::UpdateViewPointCoveredPoint(std::vector<bool>& point_list,
                                                           int viewpoint_index,
                                                           bool use_array_ind) const {
        for (const auto& point_ind :
             viewpoint_manager_->GetViewPointCoveredPointList(viewpoint_index, use_array_ind)) {
    ...
    void LocalCoveragePlanner::EnqueueViewpointCandidates(
        ...
        for (const auto& viewpoint_index : viewpoint_manager_->GetViewPointCandidateIndices()) {
            ...
            if (std::find(selected_viewpoint_array_indices.begin(),
                          selected_viewpoint_array_indices.end(), viewpoint_array_index)
    ...
    void LocalCoveragePlanner::SelectViewPointFromFrontierQueue(
        std::vector<std::pair<int, int>>& frontier_queue, std::vector<bool>& frontier_covered,
...

    {
        ...
        bool has_robot_lookahead_dummy =
            robot_ind != lookahead_ind;  // 判断机器人和lookahead point是否(不同)为dummy点
        ...
        if (has_start_end_dummy && has_robot_lookahead_dummy) {
            ...
            for (int i = 0; i < selected_viewpoint_indices.size(); i++) {
                ...
                if (i == start_ind || i == end_ind) {
                    ...
                    distance_matrix[start_end_dummy_node_ind][i] =
                        0;  // 表示dummy与start_end之间的距离为0
                } else {
                    distance_matrix[i][start_end_dummy_node_ind] = 9999;
                    distance_matrix[start_end_dummy_node_ind][i] =
                        9999;  // 表示dummy与start_end之间的距离为9999
                }
        ...
        if (path_index.size() > 1) {
            ...
            for (int i = 0; i < path_index.size() - 1; i++) {
                ...
                exploration_path_ns::Node cur_node(cur_node_position,
                ...
                // 这段存在不存似乎无伤大雅？
                geometry_msgs::Point next_node_position =
                    viewpoint_manager_->GetViewPointPosition(next_ind);
                exploration_path_ns::Node next_node(next_node_position,
                                                    exploration_path_ns::NodeType::LOCAL_VIEWPOINT);
    ...
    exploration_path_ns::ExplorationPath LocalCoveragePlanner::SolveLocalCoverageProblem(
        const exploration_path_ns::ExplorationPath& global_path, int uncovered_point_num,
        ...
        std::vector<bool> frontier_covered(uncovered_frontier_point_num,
        ...
        for (auto& viewpoint_array_ind : last_selected_viewpoint_array_indices_) {
            ...
            int covered_point_num =
                viewpoint_manager_->GetViewPointCoveredPointNum(covered, viewpoint_array_ind, true);
        ...
        if (!queue.empty() && queue[0].first > parameters_.kMinAddPointNum) {
            ...
            for (int itr = 0; itr < parameters_.kLocalPathOptimizationItrMax; itr++) {
                ...
                misc_utils_ns::UniquifyIntVector(
                    selected_viewpoint_indices_itr);  // 对被选视点集合进行去重、排序处理
