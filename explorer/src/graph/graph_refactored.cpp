...
namespace explorer
{
    ...
    void GraphRefactored::AddOneWayEdge(int from_node_index, int to_node_index, double distance)
    {
        ...
        {
            ROS_ERROR_STREAM("Graph::AddOneWayEdge: from_node_index: " << from_node_index << " not in range [0, "
                                                                       << connection_.size() - 1 << "]");
            return;
        }
        ...
        {
            ROS_ERROR_STREAM("Graph::AddOneWayEdge: to_node_index: " << to_node_index << " not in range [0, "
                                                                     << connection_.size() - 1 << "]");
            return;
        }
                                        ...
                                        std::vector<int>& node_indices) const
    {
        ...
        if (!NodeIndexInRange(from_node_index))
        {
            ROS_ERROR_STREAM("AStarSearch: from_node_index " << from_node_index << " out of range [0, "
                                                             << connection_.size() - 1 << "]");
            return -1.0;
        }
        ...
        if (!NodeIndexInRange(to_node_index))
        {
            ROS_ERROR_STREAM("AStarSearch: to_node_index " << to_node_index << " out of range [0, "
                                                           << connection_.size() - 1 << "]");
            return -1.0;
        }
        ...
        while (!open_set.empty())
        {
            ...
            for (size_t i = 0; i < neighbors.size(); i++)
            {
                ...
                if (tentative_g_score < g_score[neighbor])
                {
                    ...
                    f_score[neighbor] = g_score[neighbor] + (positions_[neighbor] - positions_[to_node_index]).norm();
