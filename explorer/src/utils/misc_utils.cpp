...
//  the overlap of the two intervals. > 0 if overlapped, < 0 otherwise.计算两个角度区间重叠度，s1(2)->e1(2)为逆时针，所有角度在[-pi, pi]
...
void UniquifyIntVector(std::vector<int>& list)
{
  ...
  it = std::unique(list.begin(), list.end());// std::unique 只是将重复元素移动到序列的末尾，并不会改变容器的大小
