...
namespace grid_world_ns
{
  ...
  public:
    ...
    // explicit会阻止隐式构造对象，比如Cell cell(1,2,3);会导致编译错误，只能显式构造：Cell cell = Cell(1,2,3) or 使用默认参数；
    ~Cell() = default;
  ...
  public:
    ...
    bool HasDirectKeyposeGraphConnection(const std::unique_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph,
                                         ...
                                         const Eigen::Vector3d& goal_position) const const;

    // 获取semi-explored的cell状态转换概率
    static static double GetSemiCellTransitionProbability(const int& cell_ind);
