...
#include "explorer/utility/pointcloud_utils.h"
// Components
#include "explorer/utility/exploration_path.h"
#include "explorer/utility/explorer_visualizer.h"
#include "explorer/interface/IGridWorld.h"
#include "explorer/interface/IKeyposeGraph.h" // 用于全局引导路径规划---->在探索过程中不断建立Node&Edge
#include "explorer/interface/ILocalCoveragePlanner.h"
#include "explorer/interface/IPlanningEnv.h" // 占据栅格地图
#include "explorer/interface/IViewpointManager.h" // 对Viewpoint进行操作，基于lidar模型和周围环境以及grid_world
...
namespace sensor_coverage_planner_3d_ns
{
    ...
    struct PlannerData
    {
        ...
        std::shared_ptr<keypose_graph_ns::IKeyposeGraph> keypose_graph_; // 关键位姿图
        std::unique_ptr<planning_env_ns::IPlanningEnv> planning_env_; // 环境映射
        std::shared_ptr<viewpoint_manager_ns::IViewpointManager> viewpoint_manager_; // 视点管理对象
        std::unique_ptr<local_coverage_planner_ns::ILocalCoveragePlanner> local_coverage_planner_; // 局部覆盖规划器
        std::unique_ptr<grid_world_ns::IGridWorld> grid_world_; // 网格世界
        std::unique_ptr<explorer_visualizer_ns::TAREVisualizer> visualizer_; // 可视化对象

        // Accessor methods for private members
        std::shared_ptr<keypose_graph_ns::IKeyposeGraph>& GetKeyposeGraph() { return keypose_graph_; }
        std::unique_ptr<planning_env_ns::IPlanningEnv>& GetPlanningEnv() { return planning_env_; }
        std::shared_ptr<viewpoint_manager_ns::IViewpointManager>& GetViewpointManager() { return viewpoint_manager_; }
        std::unique_ptr<local_coverage_planner_ns::ILocalCoveragePlanner>& GetLocalCoveragePlanner() { return local_coverage_planner_; }
        std::unique_ptr<grid_world_ns::IGridWorld>& GetGridWorld() { return grid_world_; }
        std::unique_ptr<explorer_visualizer_ns::TAREVisualizer>& GetVisualizer() { return visualizer_; }

        geometry_msgs::Point& GetRobotPosition() { return robot_position_; }
        geometry_msgs::Point& GetLastRobotPosition() { return last_robot_position_; }
        lidar_model_ns::LiDARModel& GetRobotViewpoint() { return robot_viewpoint_; }
        nav_msgs::Odometry& GetKeypose() { return keypose_; }
        exploration_path_ns::ExplorationPath& GetExplorationPath() { return exploration_path_; }
        Eigen::Vector3d& GetInitialPosition() { return initial_position_; }
        Eigen::Vector3d& GetLookaheadPoint() { return lookahead_point_; }
        Eigen::Vector3d& GetLookaheadPointDirection() { return lookahead_point_direction_; }
        Eigen::Vector3d& GetMovingDirection() { return moving_direction_; }
        std::vector<Eigen::Vector3d>& GetVisitedPositions() { return visited_positions_; }

        int& GetCurKeyposeNodeInd() { return cur_keypose_node_ind_; }
        double& GetRobotYaw() { return robot_yaw_; }
        bool& GetMovingForward() { return moving_forward_; }

        // PCL clouds
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<PlannerCloudPointType>>& GetKeyposeCloud() { return keypose_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZ>>& GetRegisteredScanStack() { return registered_scan_stack_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetRegisteredCloud() { return registered_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetLargeTerrainCloud() { return large_terrain_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetTerrainCollisionCloud() { return terrain_collision_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetTerrainExtCollisionCloud() { return terrain_ext_collision_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetViewpointVisCloud() { return viewpoint_vis_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetGridWorldVisCloud() { return grid_world_vis_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetSelectedViewpointVisCloud() { return selected_viewpoint_vis_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetExploringCellVisCloud() { return exploring_cell_vis_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetExplorationPathCloud() { return exploration_path_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetCollisionCloud() { return collision_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetLookaheadPointCloud() { return lookahead_point_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetKeyposeGraphVisCloud() { return keypose_graph_vis_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetViewpointInCollisionCloud() { return viewpoint_in_collision_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetPointCloudManagerNeighborCloud() { return point_cloud_manager_neighbor_cloud_; }
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>& GetReorderedGlobalSubspaceCloud() { return reordered_global_subspace_cloud_; }

        // Markers
        std::unique_ptr<misc_utils_ns::Marker>& GetKeyposeGraphNodeMarker() { return keypose_graph_node_marker_; }
        std::unique_ptr<misc_utils_ns::Marker>& GetKeyposeGraphEdgeMarker() { return keypose_graph_edge_marker_; }
        std::unique_ptr<misc_utils_ns::Marker>& GetNogoBoundaryMarker() { return nogo_boundary_marker_; }
        std::unique_ptr<misc_utils_ns::Marker>& GetGridWorldMarker() { return grid_world_marker_; }
