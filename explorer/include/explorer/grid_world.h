...
namespace grid_world_ns {
    ...
    public:
        ...
        void AddViewPoint(const int viewpoint_ind) {
            viewpoint_indices_.push_back(viewpoint_ind);
        }

        void AddGraphNode(const int node_ind) {
        ...
        void SetStatus(const CellStatus status) {
            status_ = status;
        }
        ...
        bool IsRobotPositionSet() const {
            return robot_position_set_;
        }

        geometry_msgs::Point GetRobotPosition() const {
        ...
        bool IsPathAddedToKeyposeGraph() const {
        ...
        //
        bool IsRoadmapConnectionPointSet() const {
    ...
    public:
        ...
        explicit GridWorld(int row_num = 1, int col_num = 1, int level_num = 1,
                           double cell_size = 6.0, double cell_height = 6.0,
                           int nearby_grid_num = 5);  // 两种构造方式->explicit拒绝隐式构造
        ~GridWorld() = default;
        // 从ros参数服务器中读取参数
        ...
        void UpdateCellKeyposeGraphNodes(
        ...
        // 判断栅格空间索引是否在栅格空间内
        bool SubInBound(const Eigen::Vector3i& sub) const {
        ...
        //  判断全局索引是否在栅格空间内
        bool IndInBound(const int ind) const {
        ...
        // 根据坐标获取栅格空间三维索引
        void GetCellSub(int& row_idx, int& col_idx, int& level_idx, double qx, double qy,
                        double qz) const;
        // 根据坐标获取栅格空间三维索引
        ...
        // 获取该位置的临近cell列表
        void GetNeighborCellIndices(const geometry_msgs::Point& position,
                                    const Eigen::Vector3i& neighbor_range);
        ...
        bool HasDirectKeyposeGraphConnection(
