...
namespace misc_utils_ns {
    ...
    bool CollinearXY(const geometry_msgs::Point& p1, const geometry_msgs::Point& p2,
                     const geometry_msgs::Point& p3, double threshold = 0.1);
    ...
    bool LineSegIntersectWithTolerance(const geometry_msgs::Point& p1,
                                       ...
                                       const geometry_msgs::Point& q2, double tolerance);
    ...
    void LinInterpPoints(const geometry_msgs::Point& p1, const geometry_msgs::Point& p2,
                         double resolution, typename pcl::PointCloud<PCLPointType>::Ptr& cloud) {
        ...
        if (point_dist < resolution) {
            ...
            const int num_points = static_cast<int>(point_dist / resolution);
            cloud->points.push_back(point1);
     ...
     * @param to_ind
     */
    void ConcatenatePath(nav_msgs::Path& path1, const nav_msgs::Path& path2, int from_ind = -1,
                         int to_ind = -1);
    /**
    ...
    void PublishCloud(const ros::Publisher& cloud_publisher, const PCLPointCloudType& cloud,
    ...
    public:
        ...
        void Stop(bool show, const std::string& unit = "ms") {
            timer_stop_ = std::chrono::high_resolution_clock::now();
            duration_ = timer_stop_ - timer_start_;
            if (show) {
                if (unit == "ms") {
                    std::cout << name_ << " takes " << GetDuration<std::chrono::milliseconds>()
                              << " ms" << '\n';
                } else if (unit == "us") {
                    std::cout << name_ << " takes " << GetDuration<std::chrono::microseconds>()
                              << " us" << '\n';
                } else if (unit == "ns") {
                    std::cout << name_ << " takes " << GetDuration<std::chrono::nanoseconds>()
                              << " ns" << '\n';
                } else if (unit == "s") {
                    std::cout << name_ << " takes " << GetDuration<std::chrono::seconds>() << " s"
                              << '\n';
                } else {
                    std::cout << "timer unit error!" << '\n';
                }
            }
        }
        ...
        int GetDuration(const std::string& unit = "ms") const {
            if (unit == "ms") {
    ...
    public:
        ...
        explicit Marker(ros::NodeHandle* nh, std::string pub_topic, std::string frame_id)
            : pub_topic_(std::move(pub_topic)), frame_id_(std::move(frame_id)) {
    ...
    bool InRange(const Eigen::Vector3i& sub,
                 const Eigen::Vector3i& max_sub = Eigen::Vector3i(INT_MAX, INT_MAX, INT_MAX),
    ...
    bool InFOV(const Eigen::Vector3d& point_position, const Eigen::Vector3d& viewpoint_position,
               double vertical_half_fov, double range);
    bool InFOVSimple(const Eigen::Vector3d& point_position,
                     const Eigen::Vector3d& viewpoint_position, double vertical_fov_ratio,
                     double range, double xy_dist_threshold = 0, double z_diff_threshold = 0,
                     bool print = false);
    ...
    double AStarSearch(const std::vector<std::vector<int>>& graph,
                       ...
                       int to_idx, bool get_path, std::vector<int>& path_indices);
    bool AStarSearchWithMaxPathLength(const std::vector<std::vector<int>>& graph,
                                      ...
                                      int from_idx, int to_idx, bool get_path,
