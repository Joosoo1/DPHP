...
namespace local_coverage_planner_ns {
    ...
    public:
        ...
        void SetViewPointManager(
            std::shared_ptr<viewpoint_manager_ns::ViewPointManager> const& viewpoint_manager) {
        ...
        exploration_path_ns::ExplorationPath SolveLocalCoverageProblem(
            const exploration_path_ns::ExplorationPath& global_path, int uncovered_point_num,
            int uncovered_frontier_point_num = 0);

        // Runtime
        int GetFindPathRuntime() const {
            return find_path_runtime_;
        }
        int GetViewPointSamplingRuntime() const {
            return viewpoint_sampling_runtime_;
        }
        int GetTSPRuntime() const {
            return tsp_runtime_;
        }

        bool IsLocalCoverageComplete() const {
    ...
    private:
        ...
        void EnqueueViewpointCandidates(
        ...
        void SelectViewPoint(const std::vector<std::pair<int, int>>& queue,
                             const std::vector<bool>& covered,
                             std::vector<int>& selected_viewpoint_indices,
                             bool use_frontier = false) const;
        ...
        exploration_path_ns::ExplorationPath SolveTSP(
