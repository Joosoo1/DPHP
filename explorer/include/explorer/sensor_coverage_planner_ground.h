...
namespace sensor_coverage_planner_3d_ns {
    ...
    struct PlannerData {
        ...
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZ>>
            registered_scan_stack_;  // 配准后的扫描点云堆栈
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            registered_cloud_;  // 配准后的点云

        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            large_terrain_cloud_;  // 大地形图点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            terrain_collision_cloud_;  // 地形碰撞检测点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            terrain_ext_collision_cloud_;  // 扩展地形碰撞检测点云

        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            viewpoint_vis_cloud_;  // 视点可视化点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            grid_world_vis_cloud_;  // 栅格世界可视化点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            selected_viewpoint_vis_cloud_;  // 被选择的视点可视化点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            exploring_cell_vis_cloud_;  // 正在探索的单元格可视化点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            exploration_path_cloud_;  // 探索路径点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            collision_cloud_;  // 碰撞检测点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            lookahead_point_cloud_;  // 视距航点点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
            keypose_graph_vis_cloud_;  // keypose图可视化点云
        std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>>
