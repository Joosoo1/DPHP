...
// Components
#include "keypose_graph/keypose_graph.h" // 用于全局引导路径规划---->在探索过程中不断建立Node&Edge
#include "planning_env/planning_env.h" // 占据栅格地图
#include "viewpoint_manager/viewpoint_manager.h" // 对Viewpoint进行操作，基于lidar模型和周围环境以及grid_world
...
#include "explorer_visualizer/explorer_visualizer.h" // 
#include "rolling_occupancy_grid/rolling_occupancy_grid.h" // 翻转占据栅格地图
...
struct PlannerData
{
  ...
  // 大多数点云用于可视化
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<PlannerCloudPointType>> keypose_cloud_;// keypose使用PointXYZRGBNormal
  ...
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> terrain_ext_collision_cloud_;// 扩展地形碰撞检测点云
  ...
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> selected_viewpoint_vis_cloud_;// 被选择的视点可视化点云
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> exploring_cell_vis_cloud_;// 正在探索的单元格可视化点云
  ...
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> viewpoint_in_collision_cloud_;// 存在碰撞的视点点云
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> point_cloud_manager_neighbor_cloud_;// 邻近点云
  std::unique_ptr<pointcloud_utils_ns::PCLCloud<pcl::PointXYZI>> reordered_global_subspace_cloud_;// 重新排序的子空间点云
...
private:
  ...
  Eigen::Quaterniond GetQuaternionFromYaw(double yaw);
};
}  // namespace sensor_coverage_planner_3d_ns