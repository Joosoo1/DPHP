...
void stateEstimationHandler(const nav_msgs::Odometry::ConstPtr& odom) {
    ...
    if (dirToGoal > PI)
        dirToGoal -= 2 * PI;
    else if (dirToGoal < -PI)
        dirToGoal += 2 * PI;

    if (autonomyMode) {
        if (odomTime > stopRotTime + minStopRotInterval && disToGoal > stopRotDis
            && (fabs(dirToGoal) > stopRotYaw1 * PI / 180.0
                || (fabs(dirToGoal) > stopRotYaw2 * PI / 180.0 && vehicleSpeed < minSpeed / 2.0))
            && !autoAdjustMode) {
            joyFwd = 0;
            joyLeft = 0;
            joyUp = 0;
        ...
        if (odomTime - autoModeTime > 0.0667) {
            if (autoAdjustMode)
                ...
                autoMode.data = desiredSpeed / maxSpeed;
    ...
    if (manualMode || (autonomyMode && autoAdjustMode)) {
        ...
        float desiredRoll = -stopVelXYGain * (manualSpeedXY * joyLeft - vehicleVelY)
            - posXYGain * lookAheadScale * manualSpeedXY * joyLeft;
        if (desiredRoll > maxRollPitch * PI / 180.0)
            desiredRoll = maxRollPitch * PI / 180.0;
        else if (desiredRoll < -maxRollPitch * PI / 180.0)
            desiredRoll = -maxRollPitch * PI / 180.0;

        float desiredPitch = stopVelXYGain * (manualSpeedXY * joyFwd - vehicleVelX)
            + posXYGain * lookAheadScale * manualSpeedXY * joyFwd;
        if (desiredPitch > maxRollPitch * PI / 180.0)
            desiredPitch = maxRollPitch * PI / 180.0;
        else if (desiredPitch < -maxRollPitch * PI / 180.0)
            desiredPitch = -maxRollPitch * PI / 180.0;
        ...
        if (joyFwdDb < joyFwd)
            joyFwdDb = joyFwd;
        else if (joyFwdDb > joyFwd + joyDeadband)
            joyFwdDb = joyFwd + joyDeadband;
        ...
        if (lookAheadDis <= 0)
            lookAheadDis = 0;
        else if (lookAheadDis < minLookAheadDis)
            lookAheadDis = minLookAheadDis;
        ...
        if (trackPathID > 0) {
            ...
            if (deltaDis > 0.001 && trackPathID < trackPathLength - 1) {
                deltaX = trackPath.poses[trackPathID + 1].pose.position.x
                    - trackPath.poses[trackPathID].pose.position.x;
                deltaY = trackPath.poses[trackPathID + 1].pose.position.y
                    - trackPath.poses[trackPathID].pose.position.y;
        ...
        if (autonomyMode) {
            if (desiredSpeed2 > slowTurnRate2 * desiredSpeed)
                desiredSpeed2 = slowTurnRate2 * desiredSpeed;
        } else {
            if (desiredSpeed2 > slowTurnRate2 * maxSpeed * joyFwd2)
                desiredSpeed2 = slowTurnRate2 * maxSpeed * joyFwd2;
        }
        if (desiredSpeed2 < minSpeed) desiredSpeed2 = minSpeed;
        if (joyFwd2 == 0 || !pathFound
            || (disToGoal < stopDis && autonomyMode && !autoAdjustMode)) {
            desiredSpeed2 = 0;
            ...
            float slowSpeed = (maxSpeed * (disToGoal - stopDis) + minSpeed * (slowDis - disToGoal))
                / (slowDis - stopDis);
        ...
        float deltaZ = 0;
        if (trackPathID > 0) { deltaZ = trackZ - trackPath.poses[trackPathID - 1].pose.position.z; }
        ...
        if (desiredYawRate > maxRateByYaw * PI / 180.0)
            desiredYawRate = maxRateByYaw * PI / 180.0;
        else if (desiredYawRate < -maxRateByYaw * PI / 180.0)
            desiredYawRate = -maxRateByYaw * PI / 180.0;
        desiredYawRate = yawRateScale * yawRatePath + yawBoostScale2 * desiredYawRate;

        float desiredVelZ = posZGain * disZ;
        if (desiredVelZ > maxVelByPosZ)
            desiredVelZ = maxVelByPosZ;
        else if (desiredVelZ < -maxVelByPosZ)
            desiredVelZ = -maxVelByPosZ;
    ...
    if (autonomyMode && waypointTest) {
        if (odomTime - waypointTime > waypointInterval) {
            ...
            if (waypointCount < waypointNum) {
                ...
                if (waypointCount == 1 || waypointCount == waypointNum - 2)
                    waypointTime -= waypointInterval / 2.0;
                waypointCount++;
            }
        }
    }
...
void pathHandler(const nav_msgs::Path::ConstPtr& path) {
    ...
    while (odomRecIDPointer != (odomSendIDPointer + 1) % stackNum) {
        ...
        if (fabs(pathTime - odomTimeStack[odomRecIDPointer])
            < fabs(pathTime - odomTimeStack[odomRecIDPointerNext])) {
            break;
        }
    ...
    for (int i = 1; i < pathLength; i++) {
        float trackX2 = cosTrackPitch * path->poses[i].pose.position.x
            + sinTrackPitch * path->poses[i].pose.position.z;
        float trackY2 = path->poses[i].pose.position.y;
        float trackZ2 = -sinTrackPitch * path->poses[i].pose.position.x
            + cosTrackPitch * path->poses[i].pose.position.z;

        trackPath.poses[trackPathRecID + i].pose.position.x =
            cosTrackYaw * trackX2 - sinTrackYaw * trackY2 + trackX;
        trackPath.poses[trackPathRecID + i].pose.position.y =
            sinTrackYaw * trackX2 + cosTrackYaw * trackY2 + trackY;
...
void joystickHandler(const sensor_msgs::Joy::ConstPtr& joy) {
    ...
    if (desiredSpeed < maxSpeed * joySpeed)
        desiredSpeed = maxSpeed * joySpeed;
    else if (desiredSpeed > maxSpeed * (joySpeed + joyDeadband))
        desiredSpeed = maxSpeed * (joySpeed + joyDeadband);
    if (desiredSpeed < minSpeed || joySpeed <= joyDeadband)
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber subStateEstimation =
        nh.subscribe<nav_msgs::Odometry>(stateEstimationTopic, 5, stateEstimationHandler);
