...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloud2) {
    ...
    while (odomPointerFront != odomPointerLast) {
        ...
        if (fabs(laserTime - odomTime[odomPointerFront])
            < fabs(laserTime - odomTime[odomPointerNext])) {
            break;
        }
...
void goalHandler(const geometry_msgs::PointStamped::ConstPtr& goal) {
    ...
    goalY = goal->point.y;
    goalZ = goal->point.z;
    // std::cout << "Goal: " << goalX << " " << goalY << " " << goalZ << std::endl;
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber subLaserCloud =
        nh.subscribe<sensor_msgs::PointCloud2>(depthCloudTopic, 5, laserCloudHandler);

    ros::Subscriber subTrackPoint =
        nh.subscribe<nav_msgs::Odometry>("/track_point_odom", 5, trackPointHandler);
    ...
    ros::Subscriber subClearSurrCloud =
        nh.subscribe<std_msgs::Empty>("/clear_surr_cloud", 5, clearSurrCloudHandler);
    ...
    ros::Publisher pubLaserCloud =
        nh.advertise<sensor_msgs::PointCloud2>("/collision_avoidance_cloud", 2);
#endif
        ...
        if (newlaserCloud) {
            ...
            *laserCloudStack[laserCloudCount] = *laserCloudDwz;
            ...
            while (pathScale >= minPathScale) {
                ...
                float relativeGoalPitch =
                    -atan2(relativeGoalZ,
                           sqrt(relativeGoalX * relativeGoalX + relativeGoalY * relativeGoalY))
                    * 180.0 / PI;
                float relativeGoalYaw = atan2(relativeGoalY, relativeGoalX) * 180.0 / PI;

                if (relativeGoalPitch < -pitchDiffLimit)
                    relativeGoalPitch = -pitchDiffLimit;
                else if (relativeGoalPitch > pitchDiffLimit)
                    relativeGoalPitch = pitchDiffLimit;
                if (relativeGoalYaw < -yawDiffLimit)
                    relativeGoalYaw = -yawDiffLimit;
                else if (relativeGoalYaw > yawDiffLimit)
                    relativeGoalYaw = yawDiffLimit;
                ...
                for (int i = 0; i < plannerCloudSize; i++) {
                    ...
                    if (x > 0
                        ...
                        float scaleY = x / gridVoxelOffsetX
                            + searchRadiusHori / gridVoxelOffsetY * (gridVoxelOffsetX - x)
                                / gridVoxelOffsetX;
                        float scaleZ = x / gridVoxelOffsetX
                            + searchRadiusVert / gridVoxelOffsetZ * (gridVoxelOffsetX - x)
                                / gridVoxelOffsetX;

                        int indX = int((gridVoxelOffsetX + gridVoxelSize / 2 - x) / gridVoxelSize);
                        int indY = int((gridVoxelOffsetY + gridVoxelSize / 2 - y / scaleY)
                                       / gridVoxelSize);
                        int indZ = int((gridVoxelOffsetZ + gridVoxelSize / 2 - z / scaleZ)
                                       / gridVoxelSize);
                        if (indX >= 0 && indX < gridVoxelNumX && indY >= 0 && indY < gridVoxelNumY
                            && indZ >= 0 && indZ < gridVoxelNumZ) {
                            int ind =
                                gridVoxelNumY * gridVoxelNumZ * indX + gridVoxelNumZ * indY + indZ;
                ...
                for (int i = 0; i < pathNum; i++) {
                    ...
                    float pitchDiff =
                        fabs(pitch + trackPitch * 180.0 / PI - vehiclePitch * 180.0 / PI
                             - depthCamPitchOffset * 180.0 / PI);
                    ...
                    if (clearPathList[i] < pointPerPathThre) {
                        ...
                        if (pitchDiff > pitchDiffLimit) { pitchDiff = pitchDiffLimit; }
                    ...
                    for (int i = 0; i < pathNum; i++) {
                        if (clearPathList[i] < pointPerPathThre) {
                            ...
                            for (int j = 0; j < freePathLength; j++) {
                                ...
                                if (dis <= (relativeGoalDis + stopDis) / pathScale
                                    || relativeGoalX < 0) {
                                    point.x *= pathScale;
                                    point.y *= pathScale;
                                    point.z *= pathScale;
